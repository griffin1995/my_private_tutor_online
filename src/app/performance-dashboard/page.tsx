...
import {
	...
	getBuildMetrics,
} from '@/app/_components/web-vitals';
			...
			<div className='max-w-7xl mx-auto'>
				...
				<div className='mb-8'>
					...
					<p className='text-gray-600'>
						Symphony Approach™ - Real-time performance metrics for £157,000 annual
						optimization value
					</p>
				...
				<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8'>
					{metrics.map((metric, index) => (
						<div
							...
							{metric.rating && (
								<span
									className={`inline-block px-2 py-1 text-xs font-medium rounded ${getRatingColor(metric.rating)}`}>
									{metric.rating}
								</span>
							)}
							{metric.target && (
						...
						<div>
							...
							<div className='w-full bg-gray-200 rounded-full h-2'>
								<div
									...
									style={{
										width: `${Math.max(0, 100 - (buildMetrics?.typeErrorCount || 0) * 2)}%`,
									}}
								/>
							</div>
						</div>
						...
						<div>
							...
							<div className='w-full bg-gray-200 rounded-full h-2'>
								<div
									...
									style={{
										width: `${Math.min(100, ((15 - (buildMetrics?.buildTime || 30)) / 15) * 100)}%`,
									}}
								/>
							</div>
						</div>
						...
						<div>
							...
							<div className='w-full bg-gray-200 rounded-full h-2'>
								<div
									...
									style={{
										width: `${Math.min(100, ((150 - (buildMetrics?.bundleSize || 200)) / 150) * 100)}%`,
									}}
								/>
							</div>
						</div>
									...
									.map((vital, index) => (
										<tr key={index}>
											...
											<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
												{vital.value < 10 ? vital.value.toFixed(3) : vital.value.toFixed(0)}
												{vital.name === 'CLS' ? '' : 'ms'}
											</td>
											<td className='px-6 py-4 whitespace-nowrap'>
												<span
													className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getRatingColor(vital.rating)}`}>
													{vital.rating || 'N/A'}
												</span>
											</td>
											<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
