...
describe('FAQAnalyticsEngine', () => {
	...
	describe('Report Generation', () => {
		...
		test('calculates overview metrics correctly', () => {
			...
			expect(report.overview.satisfactionRate).toBe(75);
			expect(report.overview.responseRate).toBe(25);
		});
		test('identifies top performing questions', () => {
			...
			expect(report.overview.topPerformingQuestions[0].helpfulPercentage).toBe(
				100,
			);
		});
		test('calculates confidence scores based on sample size', () => {
			const report = analyticsEngine.generateReport();
			const q1Stats = report.overview.topPerformingQuestions.find(
				(q) => q.questionId === 'q1',
			);
			const q2Stats = report.overview.topPerformingQuestions.find(
				(q) => q.questionId === 'q2',
			);
			expect(q1Stats?.confidence).toBeGreaterThan(q2Stats?.confidence);
		});
		...
		test('calculates performance metrics correctly', () => {
			const report = analyticsEngine.generateReport();
			expect(report.performance.conversionRates.ratingToFeedback).toBe(25);
			expect(report.performance.conversionRates.viewToRating).toBe(400);
		});
	});
