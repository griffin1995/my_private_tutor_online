...
// IMPLEMENTATION REASON: Following React best practices for comprehensive carousel configuration
...
// CONTEXT7 SOURCE: Official Motion documentation for carousel animation timing
// ANIMATION CONFIGURATION REASON: Professional animation timing following Motion best practices
const ANIMATION_SPEEDS = {
  slow: 60,    // 60 seconds for full cycle
  medium: 45,  // 45 seconds for full cycle  
  fast: 30     // 30 seconds for full cycle
} as const

// CONTEXT7 SOURCE: /context7/motion_dev - Advanced carousel animation variants
// CONTEXT7 SOURCE: Official Motion patterns for infinite scroll animations
// CAROUSEL ANIMATION REASON: Following Motion best practices for smooth infinite carousel
...
// CONTEXT7 SOURCE: /context7/motion_dev - Container animation variants for staggered reveals
// CONTEXT7 SOURCE: Official Motion documentation for container and children animations
// CONTAINER ANIMATION REASON: Professional section reveal with staggered content
...
export function EliteSchoolsCarousel({
  ...
  // CONTEXT7 SOURCE: Official React documentation for performance optimization with useMemo
  // PERFORMANCE OPTIMIZATION REASON: Filtering and processing schools only when dependencies change
  const filteredSchools = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(school => 
        ...
        school.subjects?.some(subject => subject.toLowerCase().includes(query)) ||
        school.specialisms?.some(specialism => specialism.toLowerCase().includes(query))
      )
    }

    // Sort by priority and prestige
  ...
  // CONTEXT7 SOURCE: /reactjs/react.dev - useMemo for triple schools array creation
  // CONTEXT7 SOURCE: Official React patterns for infinite scroll data preparation
  // INFINITE SCROLL REASON: Creating seamless infinite carousel by tripling the school data
  ...
  // PERFORMANCE OPTIMIZATION REASON: Preventing unnecessary re-renders in carousel controls
  const handlePlayPause = useCallback(() => {
    ...
    trackSchoolInteraction({
      ...
      timestamp: new Date(),
      metadata: { action: isPlaying ? 'pause' : 'play' }
    })
  }, [isPlaying])

  const handleDirectionChange = useCallback(() => {
    setCurrentDirection(current => current === 'forward' ? 'reverse' : 'forward')
    trackSchoolInteraction({
      schoolId: 'carousel-controls',
      interactionType: 'click',
      timestamp: new Date(),
      metadata: { action: 'direction_change', direction: currentDirection === 'forward' ? 'reverse' : 'forward' }
    })
  }, [currentDirection])

  const handleSchoolClick = useCallback((school: EliteSchool) => {
    if (showModal) {
  ...
  const handleCategoryChange = useCallback((category: SchoolCategory | 'all') => {
    setSelectedCategory(category)
    trackSchoolInteraction({
      schoolId: 'carousel-filter',
      interactionType: 'click',
      timestamp: new Date(),
      metadata: { filter_type: 'category', category }
    })
  }, [])
  ...
  // CONTEXT7 SOURCE: /tailwindlabs/tailwindcss.com - Background variant styling patterns
  // CONTEXT7 SOURCE: Official Tailwind CSS documentation for background and gradient utilities
  // STYLING REASON: Following Tailwind best practices for variant-based background styling
  ...
  return (
    <>
      <section className={`relative ${backgroundClasses[backgroundVariant]} py-20 ${className}`}>
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {showCategoryFilter && (
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      onChange={(e) => handleCategoryChange(e.target.value as SchoolCategory | 'all')}
                ...
                {showControls && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 rounded-full bg-white/80 border border-primary-200 hover:bg-white hover:border-accent-300 transition-all duration-200 backdrop-blur-sm"
                      aria-label={currentDirection === 'forward' ? 'Reverse direction' : 'Forward direction'}
                    >
          ...
          {/* CONTEXT7 SOURCE: /context7/motion_dev - Infinite carousel with sophisticated animations */}
          {/* CAROUSEL IMPLEMENTATION REASON: Following Motion best practices for smooth infinite scroll */}
          <m.div 
          ...
          >
            <m.div 
              ...
              custom={currentDirection === 'reverse' ? -ANIMATION_SPEEDS[animationSpeed] : ANIMATION_SPEEDS[animationSpeed]}
              style={{
      ...
      {/* MODAL REASON: Professional modal system for detailed school information display */}
      {showModal && (
        <SchoolModal
