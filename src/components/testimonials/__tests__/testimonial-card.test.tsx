// CONTEXT7 SOURCE: /testing-library/react-testing-library - Testimonial card testing for trust and credibility
// TESTING REASON: Testimonials are critical for building trust and credibility in premium tutoring service
...
// Mock EnhancedTestimonial type
const mockTestimonial = {
  id: 'test-testimonial-1',
  name: 'Lady Catherine Windsor',
  role: 'Parent',
  content: 'Exceptional tutoring service that helped our son achieve outstanding results. The personalized approach and expert guidance exceeded our expectations.',
  rating: 5,
  verified: true,
  location: 'London, UK',
  date: '2024-03-15',
  ...
  expandedContent: 'The detailed approach taken by My Private Tutor Online was remarkable. From the initial assessment to the final results, every step was meticulously planned and executed. Our son not only improved academically but also gained confidence that will serve him well throughout his educational journey.',
...
describe('TestimonialCard', () => {
  describe('Basic Rendering', () => {
    it('renders testimonial card with essential information', () => {
      ...
      expect(screen.getByText(/exceptional tutoring service/i)).toBeInTheDocument()
    ...
    it('displays rating stars correctly', () => {
      ...
      // Should show 5 stars for a 5-star rating
    ...
    it('shows verification badge for verified testimonials', () => {
      ...
      // Should show shield icon for verification
  ...
  describe('Interactive Features', () => {
    ...
    it('calls onClick callback when card is clicked', async () => {
      ...
      render(<TestimonialCard testimonial={mockTestimonial} onClick={onClickMock} />)
  ...
  describe('Video Testimonial Features', () => {
    it('shows video play button for video testimonials', () => {
      ...
      const playButton = screen.getByRole('button', { name: /watch video/i })
    ...
    it('handles video testimonials without video content', () => {
      ...
      const playButton = screen.queryByRole('button', { name: /watch video/i })
  ...
  describe('Layout Variants', () => {
    it('applies masonry layout styling', () => {
      render(<TestimonialCard testimonial={mockTestimonial} layout="masonry" />)
      
      const card = screen.getByRole('article')
      expect(card).toBeInTheDocument()
      // Layout-specific classes would be applied
    })

    it('applies grid layout styling', () => {
      render(<TestimonialCard testimonial={mockTestimonial} layout="grid" />)
    ...
    it('applies list layout styling', () => {
      render(<TestimonialCard testimonial={mockTestimonial} layout="list" />)
    ...
    it('applies carousel layout styling', () => {
      render(<TestimonialCard testimonial={mockTestimonial} layout="carousel" />)
  ...
  describe('Accessibility', () => {
    it('provides proper ARIA landmarks', () => {
      ...
      const card = screen.getByRole('article')
      expect(card).toBeInTheDocument()
      expect(card).toHaveAttribute('aria-label', expect.stringContaining('Lady Catherine Windsor'))
    })
    ...
    it('provides proper button labelling', () => {
      ...
      const videoButton = screen.getByRole('button', { name: /watch video/i })
    ...
    it('provides alt text for avatar images', () => {
      ...
      const avatar = screen.getByRole('img', { name: /lady catherine windsor/i })
  ...
  describe('Hover and Animation States', () => {
    it('handles hover interactions when enabled', async () => {
      const user = userEvent.setup()
      render(<TestimonialCard testimonial={mockTestimonial} enableHover={true} />)
      
      const card = screen.getByRole('article')
      await user.hover(card)
      
      // Hover effects should be enabled
      expect(card).toBeInTheDocument()
    })

    it('disables hover effects when disabled', () => {
      render(<TestimonialCard testimonial={mockTestimonial} enableHover={false} />)
  ...
  describe('Content Display Options', () => {
    it('shows full content when enabled', () => {
      render(<TestimonialCard testimonial={mockTestimonial} showFullContent={true} />)
      
      // Should show expanded content immediately
      expect(screen.getByText(/detailed approach taken/i)).toBeInTheDocument()
      
      // Should not show expand button
      const expandButton = screen.queryByRole('button', { name: /read more/i })
      expect(expandButton).not.toBeInTheDocument()
    })

    it('truncates content by default', () => {
      render(<TestimonialCard testimonial={mockTestimonial} showFullContent={false} />)
  ...
  describe('Royal Client Standards', () => {
    it('maintains premium language and presentation', () => {
      ...
      // Should show aristocratic naming
      expect(screen.getByText('Lady Catherine Windsor')).toBeInTheDocument()
      
      // Should show premium achievement
      expect(screen.getByText('Oxford University Acceptance')).toBeInTheDocument()
  ...
  describe('Error Handling', () => {
    ...
    it('handles missing image gracefully', () => {
      ...
      const avatar = screen.getByText('LC') // Initials fallback
      expect(avatar).toBeInTheDocument()
    })
    ...
    it('handles missing video URL gracefully', () => {
      ...
      // Should not show video button if URL is missing
      const playButton = screen.queryByRole('button', { name: /watch video/i })
      expect(playButton).not.toBeInTheDocument()
    })
  })
