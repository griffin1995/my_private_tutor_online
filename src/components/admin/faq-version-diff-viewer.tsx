/**
 * FAQ Version Diff Viewer - Advanced Visual Comparison Component
 * 
 * CONTEXT7 SOURCE: /reactjs/react.dev - React component patterns with advanced state management
 * CONTEXT7 SOURCE: /google/diff-match-patch - Text diffing visualization patterns
 * CONTEXT7 SOURCE: /radix-ui/primitives - Accessible UI components for diff display
 * IMPLEMENTATION REASON: Professional-grade diff viewer with multiple display modes
 * 
 * This component provides comprehensive diff viewing capabilities including:
 * - Side-by-side comparison with synchronized scrolling
 * - Inline diff view with unified format display
 * - Syntax highlighting for structured content
 * - Line-by-line navigation with jump-to-change
 * - Export functionality for diff reports
 * - Accessibility features for screen readers
 * - Performance optimized for large content diffs
 * 
 * BUSINESS REQUIREMENTS:
 * - Royal client quality standards for admin tooling
 * - Professional diff experience matching Git tools
 * - Performance optimised for large FAQ content
 * - British English throughout interface
 * - Enterprise-grade visual comparison
 */
...
// COMPONENT PROPS: Diff viewer configuration interface
...
// CONTEXT7 SOURCE: /google/diff-match-patch - Diff visualization types
// DISPLAY MODES: Different ways to visualize version differences
type DiffViewMode = 'side-by-side' | 'inline' | 'unified' | 'split'

// CONTEXT7 SOURCE: /reactjs/react.dev - React functional component with comprehensive diff display
// DIFF VIEWER: Advanced visual comparison component
export const FAQVersionDiffViewer: React.FC<FAQVersionDiffViewerProps> = ({
  ...
  // CONTEXT7 SOURCE: /reactjs/react.dev - useRef patterns for DOM element references
  ...
  // CONTEXT7 SOURCE: /reactjs/react.dev - Async function patterns for data loading
  ...
  const changeNavigationItems = useMemo(() => {
    ...
    return comparison.diff.operations.map((op, index) => ({
      index,
      type: op.type,
  ...
  const handleViewModeChange = (mode: DiffViewMode) => {
    setViewMode(mode)
    setSelectedChange(-1) // Reset selection when changing views
  }
  
  const handleChangeNavigation = (changeIndex: number) => {
    setSelectedChange(changeIndex)
    
    // Scroll to change based on view mode
  ...
  const scrollToChange = (changeIndex: number, displayMode: 'side-by-side' | 'unified') => {
    const change = changeNavigationItems[changeIndex]
    if (!change) return
    
    if (displayMode === 'side-by-side') {
      const leftElement = leftPaneRef.current?.querySelector(`[data-line="${change.lineNumber}"]`)
      const rightElement = rightPaneRef.current?.querySelector(`[data-line="${change.lineNumber}"]`)
      
      leftElement?.scrollIntoView({ behavior: 'smooth', block: 'center' })
      rightElement?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    } else {
      const unifiedElement = unifiedPaneRef.current?.querySelector(`[data-change="${changeIndex}"]`)
      unifiedElement?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
  }
...
`
  ...
  // CONTEXT7 SOURCE: /reactjs/react.dev - Conditional rendering patterns for loading states
  if (loading) {
    return (
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
              <span>
                <span className="font-medium">Base:</span> v{comparison.baseVersion.versionString}
              </span>
              <span>•</span>
              <span>
                <span className="font-medium">Target:</span> v{comparison.targetVersion.versionString}
              </span>
              {diffStats && (
                <>
                  <span>•</span>
            ...
            <div className="overflow-y-auto h-full">
              {changeNavigationItems.map((change, index) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm border-b border-gray-200 transition-colors ${
                ...
                >
                  <div className="flex items-center space-x-2">
                    <span className={`inline-flex px-2 py-1 text-xs font-medium rounded ${getChangeTypeBadgeStyle(change.type)}`}>
...
// CONTEXT7 SOURCE: /reactjs/react.dev - Component composition patterns for reusable elements
// SUPPORTING COMPONENTS: Specialized components for different diff views
...
const VersionMetadataPanel: React.FC<VersionMetadataPanelProps> = ({ title, version }) => (
  <div>
    ...
    <dl className="grid grid-cols-2 gap-2 text-xs">
      ...
      <dd>
        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded ${getChangeTypeBadgeStyle(version.metadata.changeType)}`}>
...
// UTILITY FUNCTIONS: Helper functions for diff visualization
