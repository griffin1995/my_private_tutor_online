/**
 * FAQ Version Workflow Manager - Approval & Publishing Control
 * 
 * CONTEXT7 SOURCE: /reactjs/react.dev - React component patterns with form handling
 * CONTEXT7 SOURCE: /pmndrs/zustand - State management for workflow operations
 * CONTEXT7 SOURCE: /radix-ui/primitives - Accessible form components for workflow interface
 * IMPLEMENTATION REASON: Role-based workflow management with approval chains
 * 
 * This component provides comprehensive workflow management including:
 * - Role-based permission enforcement with visual indicators
 * - Multi-stage approval process with configurable reviewers
 ...
 * - Royal client quality standards for content approval
 * - Enterprise-grade workflow automation
 * - GDPR-compliant audit trail for all actions
 * - Performance optimised for high-volume operations
 * - British English throughout workflow interfaces
 */
...
// COMPONENT PROPS: Workflow manager configuration interface
...
// CONTEXT7 SOURCE: /reactjs/react.dev - React functional component with workflow management
// WORKFLOW MANAGER: Comprehensive workflow control interface
export const FAQVersionWorkflowManager: React.FC<FAQVersionWorkflowManagerProps> = ({
  className = '',
  versions,
  selectedVersionId,
  onVersionSelect,
  ...
  // CONTEXT7 SOURCE: /reactjs/react.dev - useState patterns for component state management
  const [activeTab, setActiveTab] = useState<'pending' | 'scheduled' | 'history' | 'config'>('pending')
  ...
  // CONTEXT7 SOURCE: /pmndrs/zustand - Store integration for workflow operations
  const store = useVersionControlStore()
  const currentUser = useCurrentUser()
  const config = useVersionConfig()
  
  // CONTEXT7 SOURCE: /reactjs/react.dev - useMemo patterns for filtered data
  const workflowData = useMemo(() => {
    const pendingVersions = versions.filter(v => 
      ['draft', 'review', 'approved'].includes(v.workflow.status)
    )
    
    const scheduledPublications = store.scheduledPublications.filter(pub => 
      pub.status === 'pending'
    )
    
    const workflowHistory = versions.filter(v => 
  ...
  // CONTEXT7 SOURCE: /reactjs/react.dev - Event handler patterns for workflow actions
  const handleVersionAction = async (versionId: string, action: string, params?: any) => {
  ...
  const handleBulkAction = async (action: BulkVersionOperation['operationType'], reason?: string) => {
    ...
    try {
      ...
      if (result.status === 'completed') {
        ...
        alert(`Bulk operation completed with ${result.processedCount} successes and ${result.failedCount} failures.`)
      }
  ...
  const handleSchedulePublication = async (versionId: string, scheduledFor: string, timezone: string) => {
    try {
      const scheduleId = await store.schedulePublication(versionId, scheduledFor, timezone)
  ...
  const handleRollback = async (fromVersion: string, toVersion: string, reason: string) => {
    try {
  ...
  const canPerformAction = (action: string, version: FAQVersion): boolean => {
    ...
    switch (action) {
      ...
      case 'review':
        return versionStatus === 'review' && ['reviewer', 'admin'].includes(userRole) && permissions.canReview
      case 'approve':
        return versionStatus === 'review' && ['reviewer', 'admin'].includes(userRole) && permissions.canApprove
      case 'reject':
        return versionStatus === 'review' && ['reviewer', 'admin'].includes(userRole) && permissions.canApprove
      case 'publish':
        return versionStatus === 'approved' && ['publisher', 'admin'].includes(userRole) && permissions.canPublish
      case 'schedule':
        return versionStatus === 'approved' && ['publisher', 'admin'].includes(userRole) && permissions.canSchedule
      case 'rollback':
  ...
  // CONTEXT7 SOURCE: /reactjs/react.dev - Conditional rendering patterns for tab content
      ...
      <div className="space-y-3">
        {workflowData.pending.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No pending versions</h3>
          ...
          workflowData.pending.map(version => (
            <WorkflowVersionCard
              ...
              actionInProgress={actionInProgress === `approve-${version.id}` || actionInProgress === `reject-${version.id}` || actionInProgress === `publish-${version.id}`}
  ...
  const renderScheduledTab = () => (
    <div className="space-y-4">
      {workflowData.scheduled.length === 0 ? (
        <div className="text-center py-12 text-gray-500">
          <div className="text-4xl mb-4">📅</div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">No scheduled publications</h3>
      ...
      {workflowData.history.length === 0 ? (
        ...
        workflowData.history.map(version => (
          <WorkflowHistoryCard
            key={version.id}
            version={version}
            onRollback={permissions.canRollback ? () => {
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            <WorkflowStatusIndicator
              ...
              todayCount={workflowData.history.filter(v => {
        ...
        <div className="mt-4 border-b border-gray-200">
          <nav className="flex space-x-8">
            {[
              { id: 'pending', label: 'Pending', count: workflowData.pending.length },
              { id: 'scheduled', label: 'Scheduled', count: workflowData.scheduled.length },
              { id: 'history', label: 'History', count: workflowData.history.length },
...
// CONTEXT7 SOURCE: /reactjs/react.dev - Component composition patterns for workflow elements
// SUPPORTING COMPONENTS: Specialized components for workflow management

interface WorkflowVersionCardProps {
  readonly version: FAQVersion
  readonly isSelected: boolean
  readonly onSelect: (selected: boolean) => void
  readonly onAction: (versionId: string, action: string, params?: any) => Promise<void>
...
const WorkflowVersionCard: React.FC<WorkflowVersionCardProps> = ({
  ...
  const handleApprove = () => {
    onAction(version.id, 'approve', { reviewNotes })
    setReviewNotes('')
  ...
  const handleReject = () => {
    ...
    if (rejectionReason) {
      onAction(version.id, 'reject', { rejectionReason })
    }
  }
        ...
        <div className="flex items-start space-x-3 flex-1">
          ...
          <div className="flex-1 cursor-pointer" onClick={onClick}>
            <div className="flex items-center space-x-2">
              ...
              <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getWorkflowStatusBadgeStyle(version.workflow.status)}`}>
                {formatWorkflowStatus(version.workflow.status)}
              </span>
              {version.metadata.changeType && (
                <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getChangeTypeBadgeStyle(version.metadata.changeType)}`}>
                  {version.metadata.changeType}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center space-x-4 text-xs text-gray-500 mt-2">
              ...
              {version.workflow.submittedAt && (
                <>
                  <span>•</span>
                  <span>Submitted {new Date(version.workflow.submittedAt).toLocaleDateString('en-GB')}</span>
                </>
              )}
            </div>
          </div>
...
// ScheduledPublicationCard, WorkflowHistoryCard, WorkflowStatusIndicator, etc.

// CONTEXT7 SOURCE: /reactjs/react.dev - Utility function patterns for workflow styling
// UTILITY FUNCTIONS: Helper functions for workflow status and styling
...
// Mock components for missing dependencies - these would be implemented separately
const ScheduledPublicationCard = ({ publication, onCancel, onReschedule }: any) => <div>Scheduled Publication Card</div>
const WorkflowHistoryCard = ({ version, onRollback, onClick }: any) => <div>Workflow History Card</div>
const WorkflowStatusIndicator = ({ pendingCount, scheduledCount, todayCount }: any) => <div>Status Indicator</div>
const WorkflowConfigurationPanel = ({ config, onUpdate, permissions }: any) => <div>Configuration Panel</div>
const SchedulePublicationModal = ({ versionId, onSchedule, onClose }: any) => <div>Schedule Modal</div>
const RollbackVersionModal = ({ versions, onRollback, onClose }: any) => <div>Rollback Modal</div>
const WorkflowConfigModal = ({ config, onSave, onClose }: any) => <div>Config Modal</div>
