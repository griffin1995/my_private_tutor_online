...
export function RoyalTestimonialCard({
	...
	const containerClasses = {
		standard:
			'bg-white rounded-xl p-6 shadow-md hover:shadow-lg border border-primary-100 transition-all duration-300',
		premium:
			'bg-gradient-to-br from-white to-accent-50 rounded-2xl p-8 shadow-premium hover:shadow-gold border border-accent-100 transition-all duration-500 transform hover:-translate-y-1',
		royal:
			'bg-gradient-to-br from-white via-royal-50 to-royal-100 rounded-2xl p-8 shadow-royal hover:shadow-2xl border-2 border-royal-200 transition-all duration-500 transform hover:-translate-y-2 relative overflow-hidden',
	};
				...
				{[...Array(5)].map((_, i) => (
					<Star
						...
						className={cn(
							'w-4 h-4',
							i < rating ?
								variant === 'royal' ? 'text-royal-500 fill-royal-500'
								: variant === 'premium' ? 'text-accent-500 fill-accent-500'
