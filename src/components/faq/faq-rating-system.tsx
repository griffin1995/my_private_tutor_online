...
import { ThumbsUp, ThumbsDown, MessageSquare, Send, Check, AlertTriangle, Sparkles, Plus } from 'lucide-react';
// CONTEXT7 SOURCE: /facebook/react - Task 21 gamification integration hooks
// GAMIFICATION INTEGRATION: Import gamification tracking hooks for rating system
import { useHelpfulVoteTracker } from './faq-gamification-tracker';

// CONTEXT7 SOURCE: /colinhacks/zod - Schema validation for form feedback data
// IMPLEMENTATION REASON: Official Zod documentation patterns for feedback form validation with custom error messages
const feedbackSchema = z.object({
  ...
  improvementSuggestions: z.string()
    .max(500, { message: 'Suggestions must be less than 500 characters' })
    .optional(),
  category: z.enum(['accuracy', 'clarity', 'completeness', 'relevance', 'other'], {
    required_error: 'Please select a feedback category'
  }).optional(),
  honeypot: z.string().max(0, { message: 'Spam detection triggered' }).optional()
});
...
interface FAQRatingSystemProps {
  questionId: string;
  questionText: string;
  onRatingSubmit?: (data: FeedbackFormData & { questionId: string }) => void;
  className?: string;
}
...
export function FAQRatingSystem({ 
  ...
  // IMPLEMENTATION REASON: Official React Hook Form patterns for controlled forms with Zod resolver integration
  const {
    register,
    handleSubmit,
    watch,
  ...
  // CONTEXT7 SOURCE: /facebook/react - Task 21 gamification tracking integration
  // GAMIFICATION TRACKING: Track helpful votes for points and achievements
  ...
  // CONTEXT7 SOURCE: /facebook/react - Points earned state for visual feedback
  // POINTS DISPLAY: Show points earned from rating activity
  ...
  // CONTEXT7 SOURCE: /colinhacks/zod - Custom validation with gamification integration
  // IMPLEMENTATION REASON: Official Zod patterns with gamification tracking for enhanced user engagement
  const handleRatingClick = (rating: 'helpful' | 'not_helpful') => {
    ...
    // CONTEXT7 SOURCE: /facebook/react - Task 21 gamification tracking integration
    // GAMIFICATION TRACKING: Track vote and calculate points reward
    ...
    const newAnalytics = {
      ...
      [rating === 'helpful' ? 'helpfulCount' : 'notHelpfulCount']: 
        ratingState.analytics[rating === 'helpful' ? 'helpfulCount' : 'notHelpfulCount'] + 1
    };
    ...
    setRatingState(prev => ({
      ...
      showFeedbackForm: rating === 'not_helpful' // Show form for negative feedback
    }));

    // Store rating in localStorage (GDPR-compliant local storage)
    localStorage.setItem(`faq_rating_${questionId}`, JSON.stringify({ 
      rating, 
      timestamp: new Date().toISOString(),
      questionId 
    }));
    localStorage.setItem(`faq_analytics_${questionId}`, JSON.stringify(newAnalytics));
  };
  ...
  const onSubmit = async (data: FeedbackFormData) => {
    ...
    try {
      const submissionData = {
        ...
        gdprConsent: true // Implicit consent through form submission
      };
      ...
      // Store feedback data locally (GDPR-compliant)
      const existingFeedback = localStorage.getItem(`faq_feedback_${questionId}`) || '[]';
      const feedbackArray = JSON.parse(existingFeedback);
      feedbackArray.push(submissionData);
      localStorage.setItem(`faq_feedback_${questionId}`, JSON.stringify(feedbackArray));
      localStorage.setItem(`faq_analytics_${questionId}`, JSON.stringify(newAnalytics));
      ...
      setTimeout(() => {
        setRatingState(prev => ({ ...prev, submitSuccess: false }));
      }, 3000);
  ...
  const getRatingPercentage = () => {
    const total = ratingState.analytics.helpfulCount + ratingState.analytics.notHelpfulCount;
    if (total === 0) return 50;
    return Math.round((ratingState.analytics.helpfulCount / total) * 100);
  };

  return (
    <div className={`bg-slate-50 border border-slate-200 rounded-lg p-6 mt-6 ${className}`}>
      {/* Success Message */}
      <AnimatePresence>
        {ratingState.submitSuccess && (
          ...
          >
            ...
            <span className="text-sm">Thank you for your feedback! Your input helps us improve our content.</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="mb-4">
        ...
        {/* Rating Analytics */}
        {(ratingState.analytics.helpfulCount > 0 || ratingState.analytics.notHelpfulCount > 0) && (
          <div className="mb-3">
            <div className="flex items-center gap-2 text-sm text-slate-600 mb-1">
              <span>{getRatingPercentage()}% found this helpful</span>
              <span className="text-slate-400">
                ({ratingState.analytics.helpfulCount + ratingState.analytics.notHelpfulCount} votes)
              </span>
            </div>
        ...
        {/* CONTEXT7 SOURCE: /context7/motion_dev - Task 21 gamification reward display */}
        {/* GAMIFICATION REWARD: Visual feedback for points earned from rating */}
        <AnimatePresence>
      ...
      <AnimatePresence>
        {ratingState.showFeedbackForm && (
          ...
          >
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                {watchedRating === 'helpful' ? 'What was most helpful?' : 'How can we improve this answer?'}
                <span className="text-red-500 ml-1">*</span>
              </label>
              <textarea
                {...register('feedback')}
                rows={4}
                placeholder={watchedRating === 'helpful' 
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
              <p className="text-xs text-blue-800">
                <strong>Privacy Notice:</strong> Your feedback will be stored securely and used only to improve our FAQ content. 
                We respect your privacy and comply with GDPR regulations. You can request deletion of your data at any time by contacting 
            ...
            <div className="flex gap-3 pt-2">
              <motion.button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
                  isValid && !ratingState.isSubmitting
      ...
      {ratingState.analytics.feedbackCount > 0 && (
        <div className="mt-4 pt-4 border-t border-slate-200">
          <p className="text-xs text-slate-500">
            {ratingState.analytics.feedbackCount} user{ratingState.analytics.feedbackCount !== 1 ? 's' : ''} provided additional feedback
          </p>
        </div>
      )}
    </div>
