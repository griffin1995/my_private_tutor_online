...
describe('QuoteRequestForm', () => {
	...
	describe('Form Rendering', () => {
		...
		it('displays royal endorsement and premium branding', () => {
			...
			expect(
				screen.getByText(/complete.*discretion.*confidentiality/i),
			).toBeInTheDocument();
		});
		it('shows all required form fields', () => {
			...
			expect(
				screen.getByLabelText(/student.*age.*year.*group/i),
			).toBeInTheDocument();
			...
			expect(
				screen.getByText(/english.*language.*literature/i),
			).toBeInTheDocument();
		});
		it('displays comprehensive subject selection options', () => {
			render(<QuoteRequestForm />);
			const subjectCheckboxes = screen.getAllByRole('checkbox');
			expect(subjectCheckboxes.length).toBeGreaterThan(10);
			expect(screen.getByText(/mathematics/i)).toBeInTheDocument();
			expect(
				screen.getByText(/sciences.*biology.*chemistry.*physics/i),
			).toBeInTheDocument();
			expect(
				screen.getByText(/modern.*languages.*french.*german.*spanish/i),
			).toBeInTheDocument();
	...
	describe('Student Information Validation', () => {
		it('validates required student name field', async () => {
			...
			const submitButton = screen.getByRole('button', {
				name: /request.*personalised.*quote/i,
			});
			await user.click(submitButton);
			await waitFor(() => {
				expect(
					screen.getByText(/student.*name.*must.*be.*at.*least.*2.*characters/i),
				).toBeInTheDocument();
			});
		});
		it('validates student name format (letters, spaces, hyphens, apostrophes only)', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/student.*name.*can.*only.*contain.*letters/i),
				).toBeInTheDocument();
			});
		});
		it('validates student age selection', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*select.*student.*age/i),
				).toBeInTheDocument();
			});
		});
		it('allows optional current school field', async () => {
			const user = userEvent.setup();
			render(<QuoteRequestForm />);
			const currentSchoolInput = screen.getByLabelText(
				/current.*school.*optional/i,
			);
	...
	describe('Subject Selection', () => {
		...
		it('allows multiple subject selection and deselection', async () => {
			const user = userEvent.setup();
			render(<QuoteRequestForm />);
			const mathsCheckbox = screen.getByRole('checkbox', {
				name: /mathematics/i,
			});
		...
		it('prevents selecting more than 8 subjects', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*select.*no.*more.*than.*8.*subjects/i),
				).toBeInTheDocument();
			});
		});
	...
	describe('Contact Information Validation', () => {
		it('validates parent/guardian name format', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/parent.*guardian.*name.*can.*only.*contain.*letters/i),
				).toBeInTheDocument();
			});
		});
		it('validates email format', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*enter.*valid.*email.*address/i),
				).toBeInTheDocument();
			});
		});
		it('validates UK postcode format', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*enter.*valid.*uk.*postcode.*format/i),
				).toBeInTheDocument();
			});
		});
		...
		it('validates phone number format', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/phone.*number.*contains.*invalid.*characters/i),
				).toBeInTheDocument();
			});
		});
	});
	describe('Tutoring Preferences', () => {
		it('validates required tutoring format selection', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*select.*preferred.*tutoring.*format/i),
				).toBeInTheDocument();
			});
		});
		it('validates required frequency selection', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*select.*preferred.*frequency/i),
				).toBeInTheDocument();
			});
		});
		it('validates required start date selection', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*select.*preferred.*start.*date/i),
				).toBeInTheDocument();
			});
		});
	});
	describe('Form Submission', () => {
		const fillValidForm = async (user: ReturnType<typeof userEvent.setup>) => {
			await user.type(
				screen.getByLabelText(/student.*name/i),
				'Princess Charlotte',
			);
			...
			await user.type(
				screen.getByLabelText(/parent.*guardian.*name/i),
				'Catherine Windsor',
			);
			await user.type(
				screen.getByLabelText(/email.*address/i),
				'catherine@royal.family',
			);
		...
		it('resets form state after successful submission', async () => {
			...
			const anotherRequestButton = screen.getByRole('button', {
				name: /submit.*another.*quote.*request/i,
			});
	...
	describe('Success State', () => {
		it('displays success message with royal branding', async () => {
			...
			await waitFor(() => {
				expect(screen.getByText(/quote.*request.*received/i)).toBeInTheDocument();
				expect(
					screen.getByText(/royal.*family.*endorsed.*service/i),
				).toBeInTheDocument();
				expect(
					screen.getByText(/personalised.*quote.*contact.*you.*within.*24.*hours/i),
				).toBeInTheDocument();
			});
		});
	});
	describe('Accessibility', () => {
		it('has proper section structure with ARIA landmarks', () => {
			...
			expect(screen.getByText(/student.*information/i)).toHaveAttribute(
				'id',
				'student-info-heading',
			);
			expect(screen.getByText(/educational.*needs/i)).toHaveAttribute(
				'id',
				'educational-needs-heading',
			);
		});
		it('provides proper ARIA labels and descriptions', () => {
			...
			const subjectGroup = screen.getByRole('group', {
				name: /subjects.*required/i,
			});
	...
	describe('Premium Service Standards', () => {
		it('maintains royal and premium language throughout', () => {
			...
			expect(
				screen.getByText(/utmost.*discretion.*confidentiality/i),
			).toBeInTheDocument();
			expect(
				screen.getByText(/highest.*standards.*data.*protection/i),
			).toBeInTheDocument();
		});
		it('includes comprehensive privacy and confidentiality notices', () => {
			render(<QuoteRequestForm />);
			expect(
				screen.getByText(/complete.*confidentiality.*guaranteed/i),
			).toBeInTheDocument();
			expect(
				screen.getByText(/information.*never.*shared.*third.*parties/i),
			).toBeInTheDocument();
		});
	...
	describe('Compact Mode', () => {
		it('renders with reduced padding in compact mode', () => {
			...
			const cardHeader = screen
				.getByText(/request.*personalised.*quote/i)
