// CONTEXT7 SOURCE: /testing-library/react-testing-library - Newsletter form testing patterns for customer engagement
// TESTING REASON: Newsletter form is critical for lead generation and customer retention in tutoring business
...
// CONTEXT7 SOURCE: /testing-library/react-testing-library - Mock CMS content for consistent testing
// MOCK REASON: Isolate component logic from CMS dependencies
jest.mock('@/lib/cms/cms-content', () => ({
  getNewsletterFormContent: () => ({
    title: 'Stay Updated with Premium Educational Insights',
    description: 'Receive exclusive tutoring tips, academic news, and educational opportunities.',
...
describe('NewsletterForm', () => {
  ...
  describe('Default Variant Rendering', () => {
    it('renders with CMS-driven content', () => {
      ...
      expect(screen.getByText(/stay.*updated.*premium.*educational.*insights/i)).toBeInTheDocument()
      expect(screen.getByText(/receive.*exclusive.*tutoring.*tips/i)).toBeInTheDocument()
    ...
    it('shows marketing consent checkbox by default', () => {
      ...
      const consentCheckbox = screen.getByLabelText(/consent.*marketing.*communications/i)
    ...
    it('includes honeypot field for spam prevention', () => {
      ...
      const honeypotField = screen.getByRole('textbox', { hidden: true })
  ...
  describe('Variant Rendering', () => {
    it('renders inline variant with compact layout', () => {
      ...
      expect(screen.queryByText(/stay.*updated.*premium/i)).not.toBeInTheDocument()
      
      // Should have compact form layout
    ...
    it('renders card variant with enhanced styling', () => {
      ...
      // Should show privacy policy notice
      expect(screen.getByText(/privacy.*policy.*unsubscribe.*anytime/i)).toBeInTheDocument()
    })
    ...
    it('renders hero variant with gradient styling', () => {
      ...
      expect(submitButton).toHaveClass('bg-gradient-to-r', 'from-accent-600', 'to-accent-700')
    })
  })
  ...
  describe('Optional Features', () => {
    ...
    it('displays interest selection when enabled', () => {
      ...
      // Check for specific interest options
    ...
    it('updates visual state when interests are selected', async () => {
      ...
      const primaryEducationLabel = screen.getByLabelText(/primary.*education/i).closest('label')
      ...
      expect(primaryEducationLabel).toHaveClass('bg-accent-50', 'border-accent-300')
    })
  })
  ...
  describe('Form Validation', () => {
    it('validates required email field', async () => {
      ...
      // Should show validation error (handled by Zod schema)
      await waitFor(() => {
  ...
  describe('Form Submission', () => {
    ...
    it('handles network errors gracefully', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/network.*error.*check.*connection/i)).toBeInTheDocument()
      })
    })
  ...
  describe('Success State', () => {
    it('displays success message in default variant', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/thank.*you.*subscribing.*premium.*newsletter/i)).toBeInTheDocument()
      })
    })
    ...
    it('resets form after successful submission', async () => {
      ...
      // Form fields should be cleared (if we return to form view)
      // This would need to be tested if there's a way to return to form
    })
  })
