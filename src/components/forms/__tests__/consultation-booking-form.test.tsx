...
describe('ConsultationBookingForm', () => {
	...
	describe('Form Rendering', () => {
		it('renders all required form fields', () => {
			...
			expect(
				screen.getByRole('button', {
					name: /book.*confidential.*consultation/i,
				}),
			).toBeInTheDocument();
		});
		...
		it('shows discretion notice for premium service assurance', () => {
			...
			expect(
				screen.getByText(/highest.*standards.*discretion/i),
			).toBeInTheDocument();
		});
	});
	describe('Form Validation', () => {
		it('validates required parent name field', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/parent.*name.*must.*be.*at.*least.*2.*characters/i),
				).toBeInTheDocument();
			});
		});
		it('validates parent name format (letters, spaces, hyphens, apostrophes only)', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/parent.*name.*can.*only.*contain.*letters/i),
				).toBeInTheDocument();
			});
		});
		...
		it('validates subjects field is required', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please.*specify.*subjects.*needed/i),
				).toBeInTheDocument();
			});
		});
	});
	describe('Form Submission', () => {
		const fillValidForm = async (user: ReturnType<typeof userEvent.setup>) => {
			...
			await user.type(
				screen.getByLabelText(/subjects.*required/i),
		...
		it('submits form with valid data successfully', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/consultation.*request.*received/i),
				).toBeInTheDocument();
			});
			await waitFor(() => {
	...
	describe('Success State', () => {
		it('displays success message with royal branding', async () => {
			...
			await waitFor(() => {
				...
				expect(
					screen.getByText(/contact.*you.*within.*24.*hours/i),
				).toBeInTheDocument();
			});
		});
		it('allows submitting another enquiry from success state', async () => {
			...
			const anotherEnquiryButton = screen.getByRole('button', {
				name: /submit.*another.*enquiry/i,
			});
	...
	describe('Accessibility', () => {
		it('has proper form labelling for screen readers', () => {
			render(<ConsultationBookingForm />);
			expect(screen.getByLabelText(/parent.*guardian.*name/i)).toHaveAttribute(
				'id',
				'parentName',
			);
			expect(screen.getByLabelText(/student.*name/i)).toHaveAttribute(
				'id',
				'studentName',
			);
		...
		it('provides proper ARIA labels for select fields', () => {
			...
			expect(academicLevelSelect).toHaveAttribute(
				'aria-label',
				'Select academic level',
			);
			const timescaleSelect = screen.getByLabelText(
				/timescale.*tutoring.*support/i,
			);
			expect(timescaleSelect).toHaveAttribute(
				'aria-label',
				'Select timescale for tutoring support',
			);
		});
	...
	describe('Compact Mode', () => {
		it('renders with reduced padding in compact mode', () => {
			...
			const cardHeader = screen
				.getByText(/book.*your.*confidential.*consultation/i)
