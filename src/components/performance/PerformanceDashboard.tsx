...
// CONTEXT7 SOURCE: /vercel/next.js - Client component for performance visualization
// PERFORMANCE DASHBOARD REASON: Real-time Core Web Vitals monitoring dashboard
// CONTEXT7 SOURCE: /vercel/next.js - Performance metrics visualization patterns
// IMPLEMENTATION: Royal client-worthy performance monitoring interface

import { useState, useEffect, useCallback } from 'react';
import { webVitalsTracker, PERFORMANCE_THRESHOLDS, type WebVitalsData, type MetricName } from '@/lib/performance/web-vitals';
...
export function PerformanceDashboard({ 
  ...
  const [metrics, setMetrics] = useState<Record<MetricName, WebVitalsData>>({} as Record<MetricName, WebVitalsData>);
  const [navigationMetrics, setNavigationMetrics] = useState<Record<string, number>>({});
  const [resourceMetrics, setResourceMetrics] = useState<{ type: string; count: number; size: number }[]>([]);
  ...
  const updateMetrics = useCallback(() => {
    ...
    if (window.performance) {
      ...
      const resourceSummary = new Map<string, { count: number; size: number }>();
      
      resources.forEach(resource => {
        let type = 'other';
        if (resource.name.includes('.js')) type = 'JavaScript';
        else if (resource.name.includes('.css')) type = 'CSS';
        else if (resource.name.includes('.woff') || resource.name.includes('.ttf')) type = 'Fonts';
        else if (resource.name.match(/\.(jpg|jpeg|png|gif|webp|avif|svg)/)) type = 'Images';
        
        const current = resourceSummary.get(type) || { count: 0, size: 0 };
      ...
      setResourceMetrics(Array.from(resourceSummary.entries()).map(([type, data]) => ({
        type,
        count: data.count,
        size: data.size,
      })));
  ...
  if (process.env.NODE_ENV !== 'development' && !showDetails) {
    return null; // Don't show dashboard in production unless explicitly enabled
  }

  return (
    <div className={`performance-dashboard ${className}`}>
      {/* Overall Performance Score */}
      <div className={`performance-score p-4 rounded-lg border-2 ${getRatingColor(overallRating)} mb-6`}>
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <div className="text-3xl font-bold">{calculatePerformanceScore()}/100</div>
            <div className="text-sm">{getRatingEmoji(overallRating)} {overallRating}</div>
          </div>
        </div>
      </div>
          ...
          {Object.entries(metrics).map(([name, metric]) => (
            ...
            >
              ...
              <div className="text-xs opacity-75 mb-2">
                Threshold: {formatValue(name as MetricName, PERFORMANCE_THRESHOLDS[name as MetricName])}
              </div>
              {showDetails && (
        ...
        <div className="navigation-metrics mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {Object.entries(navigationMetrics).map(([name, value]) => (
              <div key={name} className="metric-item p-3 bg-gray-50 rounded">
        ...
        <div className="resource-metrics mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {resourceMetrics.map(({ type, count, size }) => (
              <div key={type} className="resource-item p-3 bg-gray-50 rounded">
              ...
              if (metric.rating === 'poor' || metric.rating === 'needs-improvement') {
                return (
                  <li key={name} className="flex items-start">
                    ...
                    <span>
                      <strong>{name}</strong>: {getRecommendation(name as MetricName, metric)}
                    </span>
                  </li>
                );
              }
...
function getRecommendation(name: MetricName, metric: WebVitalsData): string {
  const recommendations: Record<MetricName, (metric: WebVitalsData) => string> = {
    LCP: (m) => `Reduce server response times and optimize largest images. Current: ${Math.round(m.value)}ms, Target: <2500ms`,
    INP: (m) => `Optimize JavaScript execution and reduce main thread blocking. Current: ${Math.round(m.value)}ms, Target: <200ms`,
    CLS: (m) => `Add size attributes to images and avoid inserting content above existing content. Current: ${m.value.toFixed(3)}, Target: <0.1`,
    FCP: (m) => `Eliminate render-blocking resources and optimize critical rendering path. Current: ${Math.round(m.value)}ms, Target: <1800ms`,
    TTFB: (m) => `Improve server response time and use CDN for static assets. Current: ${Math.round(m.value)}ms, Target: <600ms`,
  };
...
export const dashboardUtils = {
  ...
  isPerformanceHealthy: (metrics: Record<MetricName, WebVitalsData>): boolean => {
  ...
  getPerformanceInsights: (metrics: Record<MetricName, WebVitalsData>) => {
    ...
    return { issues, improvements, successes };
  },
};