...
import { LazyAboutContent } from '@/lib/dynamic-imports/lazy-components';
import { LazyAboutImage } from '@/lib/dynamic-imports/lazy-components';
...
export function VariantAboutSection({
  ...
  const [currentVariant, setCurrentVariant] = useState<AboutSectionVariant>(CONTROL_VARIANT);
      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="fixed bottom-4 right-4 bg-black/80 text-white p-3 rounded text-xs max-w-xs">
            ...
            <div>Animation Mode: {currentVariant.performance.animationMode}</div>
          </div>
        )}
      </div>
    </section>
...
export const useVariantAdaptation = (variant: AboutSectionVariant) => {
  ...
  const contentConfig = useMemo(() => ({
    titleClasses: variant.content.titleStyle === 'highlighted'
    ...
    imageClasses: variant.content.imageStyle === 'circular'
    ...
    credentialsLayout: variant.content.credentialsStyle === 'text'
      ? 'flex-col space-y-2'
      : variant.content.credentialsStyle === 'minimal'
      ? 'hidden'
