// CONTEXT7 SOURCE: /gdpr/regulations - GDPR consent management component
// IMPLEMENTATION REASON: Official GDPR documentation requires explicit consent for data processing
 ...
 * CONSENT REASON: Official GDPR requires explicit, informed, and withdrawable consent
 */
export const FooterGDPRConsent = React.memo<GDPRConsentProps>(({
  ...
  // CONTEXT7 SOURCE: /gdpr/regulations - Article 7.1 Demonstrable consent
  // STATE REASON: Track consent state with timestamp for audit trail
  ...
  // CONTEXT7 SOURCE: /gdpr/regulations - Article 12 Transparent information
  // TRANSPARENCY REASON: Clear information about data processing
  const dataCategories = {
    ...
    analytics: {
      ...
      purposes: ['Usage statistics', 'Performance monitoring', 'Service improvement'],
      retention: '24 months',
      locked: false
    },
    marketing: {
  ...
  /**
   * CONTEXT7 SOURCE: /gdpr/regulations - Article 7.3 Withdrawal of consent
   * WITHDRAWAL REASON: Consent must be as easy to withdraw as to give
   */
  const handleConsentToggle = (category: 'marketing' | 'analytics', value: boolean) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem('gdpr-consent-history', JSON.stringify([...consentHistory, record]));
    }
  };

  /**
   * CONTEXT7 SOURCE: /gdpr/regulations - Article 13 Information to be provided
   * INFORMATION REASON: Provide comprehensive privacy information
   */
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <p className="text-gray-600">
            Consent (Article 6.1.a GDPR) for marketing and analytics. 
            Legitimate interest (Article 6.1.f) for necessary operations.
          </p>
        </div>
      </div>
    </div>
  ...
  /**
   * CONTEXT7 SOURCE: /gdpr/regulations - Article 7.2 Burden of proof
   * PROOF REASON: Maintain audit trail of consent
   */
  ...
  if (compact) {
    // CONTEXT7 SOURCE: /gdpr/regulations - Article 7.2 Clear and plain language
    // COMPACT REASON: Simplified consent for footer newsletter form
    return (
        ...
        {showDetails && (
          <div className="pl-7 space-y-3">
            <p className="text-xs text-gray-600">
              We'll use your email to send newsletters with educational content and service updates. 
  ...
  // Full consent management interface
  return (
        ...
        {Object.entries(dataCategories).map(([key, category]) => (
          <div key={key} className="flex items-start gap-3">
            <input
              ...
              onChange={(e) => {
                if (!category.locked) {
                  handleConsentToggle(key as 'marketing' | 'analytics', e.target.checked);
                }
              }}
            ...
            />
            <div className="flex-1">
              <label htmlFor={`consent-${key}`} className="font-medium text-black cursor-pointer">
                {category.title}
                {category.locked && (
                  <span className="ml-2 text-xs text-gray-500">(Required)</span>
                )}
              </label>
              <p id={`consent-${key}-description`} className="text-sm text-gray-600 mt-1">
...
/**
 * CONTEXT7 SOURCE: /gdpr/regulations - Article 17 Right to erasure
 * ERASURE REASON: Provide data deletion functionality
 */
export function FooterDataErasure() {
  ...
  const handleErasure = () => {
    if (typeof window !== 'undefined') {
      ...
      // In production, would also call API to delete server-side data
      setErased(true);
      setTimeout(() => {
        setErased(false);
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-red-700 mt-1">
                    This will permanently delete all your data including preferences and subscription status.
                  </p>
                </div>
              </div>
