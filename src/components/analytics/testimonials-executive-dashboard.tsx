...
const AlertsSection: React.FC<{
	alerts: ExecutiveAnalyticsDashboard['alerts'];
}> = ({ alerts }) => {
...
const PerformanceChart: React.FC<{
	data: TestimonialsPerformanceMetrics['trendsData'];
}> = ({ data }) => {
	return (
		<ResponsiveContainer
			width='100%'
			...
			<AreaChart data={data}>
				...
				<Tooltip
					formatter={(value, name) => [
						...
						: name === 'views' ? 'Views'
						: name === 'engagementScore' ? 'Engagement Score'
						: name,
					]}
				/>
				<Area
...
const SegmentBreakdownChart: React.FC<{
...
const AIInsightsSection: React.FC<{
	insights: TestimonialsAIInsights;
}> = ({ insights }) => {
	return (
							...
							<div
								...
								<div className='flex items-center justify-between text-sm'>
									...
									<span className='text-green-600 font-semibold'>
										ROI: £{rec.expectedROI.toLocaleString()}
									</span>
								</div>
							</div>
						))}
							...
							<div
								...
								<div
									className={`mt-1 w-2 h-2 rounded-full ${
										...
										: opt.priority === 'medium' ? 'bg-yellow-500'
										: 'bg-green-500'
									}`}
								/>
...
const ROIAnalysisSection: React.FC<{
	roiData: TestimonialsROIAnalysis;
}> = ({ roiData }) => {
	return (
					...
					<div className='space-y-4'>
						<div>
							<div className='flex justify-between text-sm'>
								...
								<span>
									£{roiData.investmentReturn.testimonialsInvestment.toLocaleString()}
								</span>
							</div>
							<div className='flex justify-between text-sm'>
								<span>Generated Revenue</span>
								<span className='text-green-600'>
									£{roiData.investmentReturn.generatedRevenue.toLocaleString()}
								</span>
							</div>
					...
					<div className='space-y-3'>
						<div className='flex justify-between'>
							...
							<span className='font-semibold'>
								£{roiData.revenueAttribution.toLocaleString()}
							</span>
						</div>
						<div className='flex justify-between'>
							<span className='text-sm'>Lifetime Value Impact</span>
							<span className='font-semibold'>
								£{roiData.lifetimeValueImpact.toLocaleString()}
							</span>
						</div>
						<div className='flex justify-between'>
							<span className='text-sm'>Brand Value</span>
							<span className='font-semibold'>
								£{roiData.brandValueContribution.toLocaleString()}
							</span>
						</div>
						<div className='flex justify-between border-t pt-2'>
...
export const TestimonialsExecutiveDashboard: React.FC = () => {
	...
	const loadDashboardData = async () => {
		try {
			...
			const [dashboardData, performanceMetrics, roiAnalysis, aiInsights] =
				await Promise.all([
			...
			{dashboardData && (
				<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
					<MetricCard
						...
						icon={Eye}
						trend={
							dashboardData.overview.trendDirection === 'increasing' ? 'up'
								...
								<CardContent>
									<div className='space-y-4'>
										<div className='flex justify-between'>
											...
											<span className='font-semibold'>
												{(
													dashboardData.keyMetrics.testimonialToConsultationRate * 100
												).toFixed(2)}
												%
											</span>
										</div>
										...
										<div className='flex justify-between'>
											...
											<span className='font-semibold'>
												{(
													dashboardData.keyMetrics.clientSatisfactionCorrelation * 100
												).toFixed(0)}
												%
											</span>
										</div>
									...
									<div className='space-y-4'>
										<div>
											...
											<div className='flex flex-wrap gap-2 mt-1'>
												{dashboardData.businessIntelligence.topPerformingSegments.map(
													(segment, index) => (
														<Badge
															key={index}
															variant='secondary'>
