...
export class ConversionTracker {
  ...
  private setupIntersectionObservers(): void {
    ...
    setTimeout(() => {
      ...
      const credentialsSection = document.querySelector('[role="group"][aria-labelledby="credentials-heading"]');
  ...
  private setupEngagementTracking(): void {
    ...
    const trackTimeOnSection = () => {
      ...
      if (timeOnSection >= 10000) { // 10 seconds
  ...
  private assignABTestVariant(): void {
    try {
      ...
      const existingVariant = localStorage.getItem('about_section_variant') as ABTestVariant;

      if (existingVariant && ['control', 'variant_a', 'variant_b', 'variant_c'].includes(existingVariant)) {
   ...
   */
  public trackEvent(type: ConversionEventType, additionalMetadata: Record<string, any> = {}): void {
  ...
  private getPerformanceMetrics() {
    ...
    return navigation ? {
      ...
      interactionDelay: Math.round(navigation.loadEventEnd - navigation.domContentLoadedEventEnd)
    } : undefined;
  }
...
export type { ConversionEvent, ConversionFunnelStage, ABTestConfig };