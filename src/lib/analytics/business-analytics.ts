// CONTEXT7 SOURCE: /vercel/next.js - Custom analytics library for business event tracking
// BUSINESS ANALYTICS REASON: Premium tutoring service conversion and engagement tracking
// CONTEXT7 SOURCE: /vercel/next.js - Event-driven analytics with performance integration
// IMPLEMENTATION: Royal client service analytics with comprehensive business metrics

import { webVitalsTracker } from '@/lib/performance/web-vitals';
import { BUSINESS_METRICS_CONFIG } from '../../performance.config';

// Business event types for tutoring service
...
// Tutoring service specific events
    ...
    if (typeof window !== 'undefined') {
      ...
      if (stored) {
        try {
          ...
          // Use stored session if less than 30 minutes old
...
class BusinessAnalytics {
  ...
  async track(
    ...
    const promises = this.providers
      ...
      .map(provider => provider.track(businessEvent).catch(err => 
  ...
  async identify(userId: string, traits: Record<string, any> = {}) {
    const promises = this.providers
      ...
      .map(provider => provider.identify!(userId, traits).catch(err =>
  ...
  async group(groupId: string, traits: Record<string, any> = {}) {
    const promises = this.providers
      ...
      .map(provider => provider.group!(groupId, traits).catch(err =>
  ...
  // Tutoring-specific tracking methods
  ...
  async trackInquiryFormSubmit(formType: string, subject?: string, level?: string) {
  ...
  async trackVideoEngagement(videoId: string, duration: number, completed: boolean) {
    return this.track(completed ? TutoringEvents.VIDEO_COMPLETE : TutoringEvents.VIDEO_PLAY, {
      category: 'engagement',
...
export const analyticsUtils = {
  ...
  trackTiming: (category: string, variable: string, time: number, label?: string) => {
    return businessAnalytics.track('timing', {
      ...
      value: time,
      metadata: { category, variable, label },
    });
  },
