...
export const useVersionControl = (): UseVersionControlResult => {
	...
	const updateVersion = useCallback(
		async (versionId: string, updates: Partial<FAQVersion>): Promise<void> => {
			try {
				setLoading(true);
				setError(null);
	...
	const approveVersion = useCallback(
		async (versionId: string, reviewNotes?: string): Promise<void> => {
			try {
				setLoading(true);
				setError(null);
	...
	const rejectVersion = useCallback(
		async (versionId: string, rejectionReason: string): Promise<void> => {
			try {
				setLoading(true);
				setError(null);
				...
				const errorMessage =
					err instanceof Error ? err.message : 'Failed to reject version';
				setError(errorMessage);
				throw new Error(errorMessage);
			} finally {
				setLoading(false);
	...
	const bulkOperation = useCallback(
		async (operation: BulkVersionOperation): Promise<BulkOperationResult> => {
			try {
				setLoading(true);
				setError(null);
	...
	const canPerformAction = useCallback(
		(action: string, version: FAQVersion): boolean => {
...
export const useVersionHistory = (faqId: string): UseVersionHistoryResult => {
	...
	const refresh = useCallback(async () => {
		try {
			...
			const errorMessage =
				err instanceof Error ? err.message : 'Failed to refresh version history';
			setError(errorMessage);
		} finally {
			setLoading(false);
		}
...
export const useVersionComparison = (): UseVersionComparisonResult => {
	...
	const compare = useCallback(
		async (baseVersionId: string, targetVersionId: string) => {
			try {
				setLoading(true);
				setError(null);
...
export const useVersionValidation = () => {
	const validateVersionContent = useCallback(
		(
			content: any,
		): {
			isValid: boolean;
	...
	const validateVersionMetadata = useCallback(
		(
			metadata: Partial<FAQVersion['metadata']>,
		): {
			isValid: boolean;
			...
			if (metadata.changeType && !validChangeTypes.includes(metadata.changeType)) {
				errors.push(
					'Invalid change type. Must be one of: ' + validChangeTypes.join(', '),
				);
			}
			return {
				isValid: errors.length === 0,
