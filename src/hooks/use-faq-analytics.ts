...
export function useFAQAnalytics(dateRange?: {
	start: Date;
	end: Date;
}): UseAnalyticsReturn {
	...
	const generateReport = useCallback(() => {
		try {
			...
			setError(
				err instanceof Error ? err.message : 'Failed to generate analytics report',
			);
	...
	const trackPerformance = useCallback(
		(metric: Omit<PerformanceMetric, 'timestamp'>) => {
			try {
				...
				setError(
					err instanceof Error ? err.message : 'Failed to track performance',
				);
			}
		},
		[],
...
export function usePerformanceTracking(): UsePerformanceTrackingReturn {
	...
	const endTracking = useCallback(
		(questionId: string, rating?: 'helpful' | 'not_helpful') => {
			const data = trackingData.current.get(questionId);
			if (!data) return;
			...
			faqAnalytics.trackPerformance({
				...
				bounceRate: totalDuration < 3000 && data.interactions === 0,
			});
			if (rating) {
	...
	useEffect(() => {
		...
		if (assignedGroup !== 'control' && variations.length > 0) {
			const variationIndex =
				assignedGroup === 'A' ? 0 : Math.min(1, variations.length - 1);
			setSelectedVariation(variations[variationIndex]);
		}
	}, [questionId, variations]);
	const trackTestInteraction = useCallback(
		(action: 'view' | 'rate' | 'feedback', data?: any) => {
			const testData = {
				questionId,
				testGroup,
			...
			const existingTests = JSON.parse(
				localStorage.getItem('faq_ab_tests') || '[]',
			);
