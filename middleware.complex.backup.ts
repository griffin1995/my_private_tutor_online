...
import {
	...
	validateDeepLink,
	detectPlatform,
} from '@/lib/deep-linking/url-patterns';
...
export default async function middleware(req: NextRequest) {
	...
	const shouldSkipI18n =
		...
		path.startsWith('/videos/') ||
		path.startsWith('/public/videos/') ||
		/\.(ico|png|jpg|jpeg|svg|gif|webp|css|js|woff|woff2|ttf|otf|mp4|webm|mov|avi)$/.test(
			path,
		);
	...
	const shouldTrackPerformance = PERFORMANCE_PATHS.some((apiPath) =>
