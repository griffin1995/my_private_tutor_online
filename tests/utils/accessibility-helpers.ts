 ...
 */
export async function testAccessibility(page: Page, options?: {
  tags?: string[]
  rules?: string[]
 ...
 */
export async function testKeyboardNavigation(page: Page, options?: {
...
export async function testScreenReaderAnnouncements(page: Page) {
  ...
  return {
    getAnnouncements: async () => {
      return await page.evaluate(() => (window as any).__screenReaderAnnouncements || [])
    }
  }
}
...
export async function testColorContrast(page: Page, selectors?: string[]) {
  const contrastIssues = await page.evaluate((selectorsToTest) => {
    ...
    const elementsToTest = selectorsToTest?.length 
      ? selectorsToTest.map(selector => document.querySelector(selector)).filter(Boolean)
