	...
	describe('Communication System', () => {
		...
		it('formats royal client communications correctly', () => {
			const formatClientCommunication = (clientData: any, messageType: string) => {
				...
				const salutation =
					salutations[clientData.clientType as keyof typeof salutations] || 'Dear';
				...
				return {
					...
					responseTimeExpected:
						clientData.clientType === 'royal-family' ? '2 hours' : '24 hours',
				};
			};
			...
			const communication = formatClientCommunication(
				royalClient,
