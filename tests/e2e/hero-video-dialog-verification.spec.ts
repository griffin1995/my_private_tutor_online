...
test.describe('HeroVideoDialog Verification Test Suite', () => {
  ...
  test('Should capture initial page state and verify page structure', async () => {
    ...
    const videoThumbnails = await page.locator('[data-testid*="video-thumbnail"], .video-thumbnail, [class*="video"], [class*="thumbnail"]').count();
    console.log(`📹 Found ${videoThumbnails} video thumbnails on page`);
    
    // Verify essential page elements
    const pageHeading = page.locator('h1, [class*="hero"], [class*="title"]').first();
  ...
  test('Should test free video thumbnails (YouTube videos) functionality', async () => {
    ...
    const freeVideos = page.locator('[class*="free"], [data-type="free"], :has-text("Free")').first();
    ...
    if (await freeVideos.count() > 0) {
      ...
      const modal = page.locator('[role="dialog"], .modal, [class*="dialog"], [class*="modal"]');
      ...
      const youtubeFrame = page.locator('iframe[src*="youtube"], iframe[src*="youtu.be"]');
      ...
      console.log(`✅ Free video modal opened successfully (YouTube: ${hasYouTube}, Video: ${hasVideo})`);
      
      // Test modal close with X button
      const closeButton = page.locator('[aria-label*="close"], .close, [class*="close"]').first();
  ...
  test('Should test paid video thumbnails (MP4 videos) functionality', async () => {
    ...
    const videoThumbnails = page.locator('[class*="thumbnail"], [class*="video"], [data-testid*="video"]');
    ...
    if (thumbnailCount > 0) {
      ...
      try {
        ...
        console.log(`✅ Paid video modal opened (Video: ${hasVideo}, Controls: ${hasControls})`);
  ...
  test('Should verify modal close functionality with all methods', async () => {
    ...
    if (thumbnailCount > 0) {
      ...
      if (await modal.count() > 0 && await modal.isVisible()) {
        const closeButton = page.locator('[aria-label*="close"], .close, [class*="close"], button:has-text("×")').first();
        if (await closeButton.count() > 0) {
          await closeButton.click();
          await page.waitForTimeout(500);
          console.log('✅ Modal closed with X button');
  ...
  test('Should check for JavaScript errors and verify success criteria', async () => {
    console.log('🔍 Checking for JavaScript errors and verifying success criteria...');
    ...
    const freeBadges = await page.locator(':has-text("Free"), [class*="free"]').count();
    const paidBadges = await page.locator(':has-text("Paid"), [class*="paid"], [class*="premium"]').count();
    const durationText = await page.locator('[class*="duration"], :has-text("min"), :has-text(":")').count();
    ...
    console.log(`🏆 OVERALL VERIFICATION: ${overallSuccess ? 'PASSED' : 'NEEDS ATTENTION'}`);
