...
test.describe('Video Masterclasses - Post-Fix Validation', () => {
	...
	test('Validate video thumbnail images load correctly', async ({ page }) => {
		...
		for (let i = 0; i < thumbnailContainers.length; i++) {
			...
			const containerText =
				(await container.getAttribute('aria-label')) || `Thumbnail ${i + 1}`;

			// Check if image is loaded (no skeleton loader)
			const skeletonLoader = container.locator('.animate-pulse');
			const hasSkeletonLoader = await skeletonLoader
				.isVisible()
				.catch(() => false);
			...
			const status =
				!hasSkeletonLoader && hasImage && imageLoaded && hasPlayButton ? 'success'
				: hasImage && hasPlayButton ? 'warning'
				: 'failed';

			console.log(
				`Thumbnail ${i + 1}: ${status} - Image: ${hasImage}, Loaded: ${imageLoaded}, Skeleton: ${hasSkeletonLoader}, Play: ${hasPlayButton}`,
			);
		...
		const successfulThumbnails = validationResults.filter(
			(r) => r.type === 'thumbnail' && r.status === 'success',
		).length;
		const totalThumbnails = validationResults.filter(
			(r) => r.type === 'thumbnail',
		).length;

		console.log(
			`✅ Thumbnail success rate: ${successfulThumbnails}/${totalThumbnails} (${Math.round((successfulThumbnails / totalThumbnails) * 100)}%)`,
		);
	...
	test('Validate video modal functionality', async ({ page }) => {
		...
		const clickableVideos = await page
			.locator('.video-thumbnail-container, button:has-text("Watch")')
			.all();
		...
		for (let i = 0; i < Math.min(clickableVideos.length, 3); i++) {
			...
			const elementText =
				(await videoElement.getAttribute('aria-label')) || `Video ${i + 1}`;
			...
			try {
				...
				if (modalVisible) {
					...
					const errorMessage = modal.locator(
						':has-text("Video Error"), :has-text("Coming Soon"), :has-text("Premium Content")',
					);
		...
		const successfulModals = validationResults.filter(
			(r) =>
				r.type === 'modal' && (r.status === 'success' || r.status === 'warning'),
		).length;
		const totalModals = validationResults.filter(
			(r) => r.type === 'modal',
		).length;

		console.log(
			`✅ Modal success rate: ${successfulModals}/${totalModals} (${Math.round((successfulModals / totalModals) * 100)}%)`,
		);
	...
	test('Validate CTA button functionality', async ({ page }) => {
		...
		const successfulButtons = validationResults.filter(
			(r) => r.type === 'cta-button' && r.status === 'success',
		).length;
		const totalButtons = validationResults.filter(
			(r) => r.type === 'cta-button',
		).length;

		console.log(
			`✅ CTA button success rate: ${successfulButtons}/${totalButtons} (${Math.round((successfulButtons / totalButtons) * 100)}%)`,
		);
	...
	test('Validate video sources are accessible', async ({ page }) => {
		...
		const successfulSources = validationResults.filter(
			(r) => r.type === 'video-source' && r.status === 'success',
		).length;
		const totalSources = validationResults.filter(
			(r) => r.type === 'video-source',
		).length;

		console.log(
			`✅ Video source success rate: ${successfulSources}/${totalSources} (${Math.round((successfulSources / totalSources) * 100)}%)`,
		);
...
async function generatePostFixReport(): Promise<void> {
	...
	// Breakdown by type
	const typeBreakdown = validationResults.reduce(
		(acc, result) => {
			if (!acc[result.type]) {
		...
		{} as Record<
			string,
			{ success: number; warning: number; failed: number; total: number }
		>,
	);
	...
	Object.entries(typeBreakdown).forEach(([type, stats]) => {
		...
		console.log(
			`   ${type}: ${stats.success}/${stats.total} (${successPercent}%) successful`,
		);
	...
	const successfulFixes = validationResults.filter(
		(r) => r.status === 'success',
	);
	...
	if (successRate >= 80) {
		console.log(
			`   🎯 EXCELLENT: ${successRate}% success rate - Video links are working well!`,
		);
	} else if (successRate >= 60) {
		console.log(
			`   ✅ GOOD: ${successRate}% success rate - Most video links are working`,
		);
	} else if (successRate >= 40) {
		console.log(
			`   ⚠️  MODERATE: ${successRate}% success rate - Some improvements made`,
		);
	} else {
		console.log(
			`   ❌ NEEDS WORK: ${successRate}% success rate - Additional fixes needed`,
		);
	}
