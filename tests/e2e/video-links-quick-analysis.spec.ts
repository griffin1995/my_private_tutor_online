...
test.describe('Video Masterclasses - Quick Video Link Analysis', () => {
  ...
  test('Identify video elements and test basic functionality', async ({ page }) => {
    ...
    const videoThumbnails = await page.locator('.video-thumbnail-container').all();
    console.log(`Found ${videoThumbnails.length} video thumbnail containers`);
    
    // Check for play buttons
    const playButtons = await page.locator('.video-play-button, button:has(svg)').all();
    ...
    if (videoThumbnails.length > 0) {
      ...
      try {
        ...
        const openModal = await page.locator('[role="dialog"]:visible, .modal:visible').first();
  ...
  test('Check specific video sources in CMS', async ({ page }) => {
    ...
    console.log(`Found ${videoElements.length} video elements with src attributes`);
    ...
    const iframes = await page.locator('iframe[src*="video"], iframe[src*="youtube"]').all();
  ...
  test('Test video modal state management', async ({ page }) => {
    ...
    const hasVideoModalState = await page.evaluate(() => {
      ...
      const reactFiber = (window as any).React || (document.querySelector('#__next') as any)?._reactInternalInstance;
      return !!reactFiber;
    });
    ...
    const modalElements = await page.locator('.fixed.inset-0, [role="dialog"]').all();
    console.log(`Modal elements in DOM: ${modalElements.length}`);
    
    // Check for video loading states
    const loadingElements = await page.locator('.animate-pulse, .loading, .spinner').all();
