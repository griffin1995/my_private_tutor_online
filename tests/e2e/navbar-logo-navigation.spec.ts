...
class NavbarLogoPage {
  ...
  get navbar() {
    return this.page.locator('[data-testid="navbar-container"], nav.fixed, .navbar-container').first()
  }
  ...
  get mobileMenuToggle() {
    return this.page.locator('button[aria-label*="menu"], button[data-testid="mobile-menu-toggle"]').first()
  }
  ...
  async checkNavbarTransparency() {
    ...
    return classList?.includes('bg-transparent') || classList?.includes('transparent')
  }
...
test.describe('Navbar Logo Navigation - Critical E2E Paths', () => {
  ...
  test.describe('Logo Visibility and Navigation', () => {
    test('should display logo correctly on homepage load', async ({ page }) => {
      ...
      await expect(navbarPage.logoLink).toHaveAttribute('aria-label', expect.stringMatching(/My Private Tutor Online.*navigate/i))
    })
    ...
    test('should maintain logo functionality across different pages', async ({ page }) => {
      const pages = [
  ...
  test.describe('Scroll-Based Logo Switching', () => {
    test('should switch logo based on scroll position on non-homepage', async ({ page }) => {
      await navbarPage.navigateToServicesPage()
    ...
    test('should maintain standard logo on homepage regardless of scroll', async ({ page }) => {
      await navbarPage.navigateToHomepage()
    ...
    test('should handle rapid scroll events without logo switching issues', async ({ page }) => {
      await navbarPage.navigateToServicesPage()
  ...
  test.describe('Responsive Logo Behavior', () => {
    ...
    viewports.forEach(({ name, width, height }) => {
      test(`should display logo correctly on ${name} viewport`, async ({ page }) => {
  ...
  test.describe('Performance and Loading', () => {
    ...
    test('should handle logo switching without layout shift', async ({ page }) => {
      await navbarPage.navigateToServicesPage()
    ...
    test('should maintain 60fps scroll performance with logo transitions', async ({ page }) => {
      await navbarPage.navigateToServicesPage()
  ...
  test.describe('Accessibility Compliance', () => {
    test('should meet keyboard navigation requirements', async ({ page }) => {
      ...
      const logoIsFocusable = focusedHref === '/' || await navbarPage.logoLink.isEnabled()
    ...
    test('should support high contrast mode', async ({ page }) => {
      ...
      await page.emulateMedia({ prefersColorScheme: 'dark', prefersReducedMotion: 'reduce' })
