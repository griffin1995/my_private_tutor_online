...
async function extractQuoteTypography(
	page: Page,
	viewport: string,
): Promise<TypographyData['quoteSections']> {
...
async function captureQuoteSectionScreenshots(
	page: Page,
	pageName: string,
	viewport: string,
) {
	...
	await page.screenshot({
		path: join(
			screenshotDir,
			`${pageName.toLowerCase().replace(/\s+/g, '-')}-${viewport}-full-page.png`,
		),
		fullPage: true,
	});
			...
			if (isVisible) {
				try {
					await element.screenshot({
						path: join(
							screenshotDir,
							`${pageName.toLowerCase().replace(/\s+/g, '-')}-${viewport}-quote-${selector.replace(/[^a-zA-Z0-9]/g, '-')}-${i}.png`,
						),
					});
				} catch (error) {
...
test.describe('QuoteSection Typography Consistency Testing', () => {
	...
	test.describe('Desktop Viewport (1440x900)', () => {
		...
		for (const pageConfig of PAGES_TO_TEST) {
			test(`Check QuoteSection typography on ${pageConfig.name}`, async ({
				page,
			}) => {
				...
				if (quoteSections.length > 0) {
					...
					console.log(
						`✅ ${pageConfig.name} (Desktop): Found ${quoteSections.length} quote section(s)`,
					);
				} else {
	...
	test.describe('Mobile Viewport (375x667)', () => {
		...
		for (const pageConfig of PAGES_TO_TEST) {
			test(`Check QuoteSection typography on ${pageConfig.name} mobile`, async ({
				page,
			}) => {
				const fullUrl = BASE_URL + pageConfig.path;
				...
				if (quoteSections.length > 0) {
					...
					console.log(
						`✅ ${pageConfig.name} (Mobile): Found ${quoteSections.length} quote section(s)`,
					);
				} else {
	...
	test.afterAll(async () => {
		...
		console.log(
			`🖼️  Screenshots: ${join(resultsDir, 'quote-section-screenshots')}`,
		);
	});
});
...
function generateTypographySummary(results: TypographyData[]): string {
	...
	const totalQuotes = results.reduce(
		(sum, page) => sum + page.quoteSections.length,
		0,
	);
	const pagesWithQuotes = results.filter(
		(page) => page.quoteSections.length > 0,
	).length;
	...
	const fontSizes = results.flatMap((page) =>
		page.quoteSections.map((quote) => ({
			page: page.page,
			fontSize: quote.computedFontSize,
	...
	if (totalQuotes === 0) {
		...
		const desktopInconsistencies =
			desktopSizes.length > 0 &&
			[...new Set(desktopSizes.map((f) => f.fontSize))].length > 1;
		const mobileInconsistencies =
			mobileSizes.length > 0 &&
			[...new Set(mobileSizes.map((f) => f.fontSize))].length > 1;
