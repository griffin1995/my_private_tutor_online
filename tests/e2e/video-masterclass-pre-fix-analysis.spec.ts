...
test.describe('Video Masterclasses Page - Pre-Fix Video Link Analysis', () => {
	...
	test('Comprehensive video element discovery and analysis', async ({
		page,
	}) => {
	...
	test('Video modal functionality analysis', async ({ page }) => {
		...
		const modalTriggers = await page
			.locator(
				'button[data-video], button[data-video-id], .video-thumbnail-container, .video-play-button',
			)
			.all();
		...
		for (let i = 0; i < Math.min(modalTriggers.length, 10); i++) {
			...
			try {
				...
				for (const modalSelector of modalSelectors) {
					...
					if (await modal.isVisible().catch(() => false)) {
						...
						if ((await videoInModalElement.count()) > 0) {
							videoInModal = true;
							const videoSrc = await videoInModalElement
								.getAttribute('src')
								.catch(() => null);
				...
				videoTestResults.push({
					...
					error:
						modalFound ?
							videoInModal ? undefined
							:	'Modal opened but no video'
						:	'Modal did not open',
					clickable: true,
...
async function testVideoElement(
	...
	try {
		...
		const dataVideoId = await element
		...
		if (clickable) {
			try {
				...
				const modal = page.locator(
					'[role="dialog"], .modal, .video-modal, .fixed.inset-0',
				);
				if (await modal.isVisible().catch(() => false)) {
					modalOpens = true;
...
async function generateVideoAnalysisReport(): Promise<void> {
	...
	const successfulElements = videoTestResults.filter(
		(r) => r.status === 'success',
	);
	...
	failedElements.forEach((result, index) => {
		...
		console.log(
			`     Clickable: ${result.clickable}, Modal Opens: ${result.modalOpens}, Has Video: ${result.hasVideo}`,
		);
	});

	// Categorize failures
	const categoryBreakdown = failedElements.reduce(
		(acc, result) => {
			const category =
				result.error?.includes('clickable') ? 'Not Clickable'
