...
test.describe('Video Masterclass Page - Round 1: Baseline Testing', () => {
  ...
  test('should load video masterclass page successfully', async ({ page }) => {
    ...
    // Check hero section
    await expect(page.locator('[data-testid="simple-hero"], .hero-section, h1')).toBeVisible();
    await expect(page.locator('h2, .subtitle, [data-subtitle]')).toContainText('Expert guidance from Elizabeth Burrows');
  });
  ...
  test('should display all masterclass sections correctly', async ({ page }) => {
    ...
    await expect(page.locator('h2').filter({ hasText: 'Featured Masterclasses' })).toBeVisible();
    await expect(page.locator('h2').filter({ hasText: 'Elizabeth\'s Essential UCAS Guide' })).toBeVisible();
    await expect(page.locator('h2').filter({ hasText: 'British Culture & Literary Classics' })).toBeVisible();
  ...
  test('should display video masterclass cards with correct content', async ({ page }) => {
    await page.goto(MASTERCLASS_PAGE_URL);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      await expect(card.locator('[class*="badge"], .bg-\\[\\#CA9E5B\\], .bg-green-600')).toBeVisible(); // Price badge
    }
  });
  ...
  test('should handle video click interactions correctly', async ({ page, context }) => {
    await page.goto(MASTERCLASS_PAGE_URL);
    ...
    const payButton = page.locator('button').filter({ hasText: 'Get Access' }).first();
  ...
  test('should maintain brand colors and visual consistency', async ({ page }) => {
    await page.goto(MASTERCLASS_PAGE_URL);
    await page.waitForLoadState('networkidle');
    
    // Check brand colors are applied
    const brandElements = page.locator('[class*="#3F4A7E"], [class*="text-\\[\\#3F4A7E\\]"]');
    await expect(brandElements).toHaveCount({ min: 5 }); // Multiple brand color elements
    
    const accentElements = page.locator('[class*="#CA9E5B"], [class*="bg-\\[\\#CA9E5B\\]"]');
...
test.describe('Video Masterclass Page - Screenshot Documentation', () => {
  ...
  test('should capture baseline screenshots for iteration comparison', async ({ page }) => {
    await page.goto(MASTERCLASS_PAGE_URL);
    await page.waitForLoadState('networkidle');
    ...
    await page.locator('.hero-section, [data-testid="simple-hero"]').first().screenshot({
      path: './.playwright-mcp/video-masterclass-baseline-hero.png'
    });
    
    // Featured masterclasses section
    await page.locator('h2').filter({ hasText: 'Featured Masterclasses' }).locator('..').screenshot({
      path: './.playwright-mcp/video-masterclass-baseline-featured.png'
    });
    
    // UCAS guide section
    await page.locator('h2').filter({ hasText: 'Elizabeth\'s Essential UCAS Guide' }).locator('..').screenshot({
      path: './.playwright-mcp/video-masterclass-baseline-ucas.png'
    });
    
    // British culture section
    await page.locator('h2').filter({ hasText: 'British Culture & Literary Classics' }).locator('..').screenshot({
