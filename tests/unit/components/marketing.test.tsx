...
describe('Marketing Components', () => {
	describe('RoyalTrustIndicators', () => {
		...
		test('renders descriptions when showDescription is true', () => {
			...
			expect(
				screen.getByText('Featured in Tatler Address Book'),
			).toBeInTheDocument();
		...
		test('hides descriptions when showDescription is false', () => {
			...
			expect(
				screen.queryByText('Trusted by royal families'),
			).not.toBeInTheDocument();
			expect(
				screen.queryByText('Featured in Tatler Address Book'),
			).not.toBeInTheDocument();
		});
		...
		test('has proper accessibility attributes', () => {
			...
			const region = screen.getByRole('region', {
				name: 'Trust indicators and credentials',
			});
	...
	describe('RoyalTestimonialCard', () => {
		const mockTestimonial = {
			quote:
				"Exceptional tutoring that transformed our daughter's academic performance.",
		...
		test('renders testimonial content correctly', () => {
			...
			expect(
				screen.getByText(mockTestimonial.quote, { exact: false }),
			).toBeInTheDocument();
		...
		test('displays avatar image with proper alt text', () => {
			...
			const avatar = screen.getByRole('img', {
				name: `${mockTestimonial.author} avatar`,
			});
		...
		test('has proper accessibility structure', () => {
			...
			const article = screen.getByRole('article', {
				name: `Testimonial from ${mockTestimonial.author}`,
			});
	...
	describe('VideoThumbnailMidCard', () => {
		...
		test('renders all features with checkmarks', () => {
			...
			const checkmarks =
				screen.getAllByTestId || screen.container.querySelectorAll('svg');
			expect(checkmarks.length).toBeGreaterThanOrEqual(
				mockService.features.length,
			);
		});
		...
		test('calls onCTAClick when CTA button is clicked', () => {
			...
			const ctaButton = screen.getByRole('button', {
				name: `${mockService.ctaText} for ${mockService.title}`,
			});
		...
		test('has proper accessibility structure', () => {
			...
			expect(ctaButton).toHaveAttribute(
				'aria-label',
				`${mockService.ctaText} for ${mockService.title}`,
			);
		});
		...
		test('icon has proper accessibility attributes', () => {
			...
			const icon = screen.getByRole('img', {
				name: `${mockService.title} service icon`,
			});
