...
async function showSystemStatus(options) {
  ...
  try {
    ...
    console.log(tsResult.success ? '✅ TypeScript compilation successful' : '❌ TypeScript errors found');
    ...
    console.log(`📊 Budget utilization: ${((buildTime / 15000) * 100).toFixed(1)}% (Budget: 15s)`);

    // Check pre-commit hooks
    console.log('\n🔗 Pre-commit Hooks:');
    const hooksInstalled = await checkPreCommitHooksInstalled();
    console.log(hooksInstalled ? '✅ Pre-commit hooks installed' : '❌ Pre-commit hooks not installed');

    // Check system files
    console.log('\n📁 System Files:');
    const files = [
      { path: '.pre-commit-config.yaml', name: 'Pre-commit configuration' },
      { path: '.github/workflows/typescript-validation.yml', name: 'CI/CD workflow' },
      { path: 'src/lib/monitoring/typescript-error-monitor.ts', name: 'Error monitor' },
      { path: 'src/lib/monitoring/automated-recovery.ts', name: 'Recovery system' },
      { path: 'src/lib/monitoring/error-prevention-dashboard.ts', name: 'Dashboard' },
      { path: 'src/lib/monitoring/performance-regression-detector.ts', name: 'Performance detector' }
    ];
    ...
    // Overall status
    console.log('\n📈 Overall Status:');
    const overallHealthy = tsResult.success && buildResult.success && hooksInstalled;
    console.log(`🏥 System Health: ${overallHealthy ? '✅ Healthy' : '⚠️ Needs Attention'}`);
...
async function runManualCheck(options) {
  ...
  try {
    ...
    console.log(tsResult.success ? '✅ No TypeScript errors' : '❌ TypeScript errors found');
    ...
    if (!tsResult.success && options.includes('--recovery')) {
      ...
      console.log(retryResult.success ? '✅ Recovery successful' : '❌ Recovery failed');
    }
    ...
    console.log(buildResult.success ? '✅ Build successful' : '❌ Build failed');
    console.log(`📊 Performance: ${buildDuration < 15000 ? 'Within budget' : 'Exceeds budget'}`);
...
async function runPerformanceBenchmark(options) {
  ...
  try {
    ...
    console.log(`✅ Status: ${results.incrementalBuild.success ? 'SUCCESS' : 'FAILED'}`);
    ...
    if (options.includes('--full')) {
      ...
      console.log(`✅ Status: ${results.fullBuild.success ? 'SUCCESS' : 'FAILED'}`);
    }
    ...
    try {
      ...
      console.log(`📈 Budget: ${((bundleStats.size / 153600) * 100).toFixed(1)}% (150KB limit)`);
    } catch (error) {
    ...
    const buildUtilization = (results.incrementalBuild.duration / buildBudget) * 100;
    ...
    const benchmarkData = {
      ...
      budgetUtilization: {
        ...
        bundleSize: results.bundleSize ? (results.bundleSize.size / bundleBudget) * 100 : 0
      }
    };

    await fs.mkdir('./logs', { recursive: true });
    await fs.writeFile('./logs/performance-benchmark.json', JSON.stringify(benchmarkData, null, 2));
...
async function monitorBuild(options) {
  ...
  try {
    ...
    console.log(`📊 Performance: ${duration < 15000 ? 'Within budget' : 'Exceeds budget'}`);
    ...
    await fs.mkdir('./logs', { recursive: true });
    await fs.writeFile('./logs/build-monitoring.json', JSON.stringify(monitoringData, null, 2));
  ...
  try {
    ...
    const dashboardData = {
      ...
      prevention: {
        ...
        cicdWorkflow: await checkFileExists('.github/workflows/typescript-validation.yml'),
...
async function generateHealthReport(options) {
  ...
  try {
    ...
    // Generate summary
    report.summary = generateHealthSummary(report.status, overallScore, report.components);
    ...
    report.recommendations = await generateHealthRecommendations(report.components);
    ...
    report.businessImpact = {
      ...
      riskMitigation: report.status === 'healthy' ? 'High' : report.status === 'warning' ? 'Medium' : 'Low'
    };
    ...
    console.log(`💰 Business Value Protected: ${report.businessImpact.valueProtected}`);
    console.log(`🛡️ Prevention Effectiveness: ${report.businessImpact.preventionEffectiveness}`);
...
async function installPreCommitHooks(options) {
  ...
  try {
    ...
    const preCommitInstalled = await execCommand('which pre-commit').then(() => true).catch(() => false);
...
async function validateSystem(options) {
  ...
  try {
    ...
    const configFiles = [
      ...
      { file: '.github/workflows/typescript-validation.yml', description: 'CI/CD workflow' },
    ...
    const tsConfigValid = await execCommand('npx tsc --showConfig').then(() => true).catch(() => false);
    validationResults.push({
    ...
    console.log(`🏥 System Health: ${successRate >= 90 ? 'Excellent' : successRate >= 75 ? 'Good' : successRate >= 50 ? 'Fair' : 'Poor'}`);
    ...
    await fs.mkdir('./logs', { recursive: true });
    await fs.writeFile('./logs/validation-report.json', JSON.stringify(validationReport, null, 2));
...
The automated error prevention system is ${status === 'healthy' ? 'operating optimally' : status === 'warning' ? 'functioning with minor issues' : 'experiencing critical issues that require immediate attention'}.`;
}
