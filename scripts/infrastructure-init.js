...
const { defaultBackupManager, defaultBackupScheduler } = require('../src/lib/infrastructure/database-backup');
const { createInfrastructureMonitor } = require('../src/lib/infrastructure/monitoring');
...
async function initializeInfrastructure() {
  try {
    ...
    if (healthStatus.services.database.status === 'up') {
      ...
      console.log(`   Response time: ${healthStatus.services.database.responseTime}ms`);
    } else {
    ...
    try {
      ...
      if (testResponse.ok) {
        ...
        console.log('⚠️  Error reporting system not accessible (may be normal during initialization)');
      }
    } catch (error) {
      console.log('⚠️  Error reporting endpoint not available (start the application server first)');
    }

    // Step 8: Display system status
    console.log('\n📈 System Status Summary:');
    console.log('========================');
    console.log(`Database Status: ${healthStatus.services.database.status.toUpperCase()}`);
    console.log(`Backup System: ${healthStatus.services.backup.status.toUpperCase()}`);
    console.log(`Monitoring: ACTIVE`);
    console.log(`Error Reporting: CONFIGURED`);
    
    console.log('\n🎯 Recovery Objectives:');
    console.log(`RTO (Recovery Time): ${healthStatus.rtoRpoMetrics.recoveryTime} minutes`);
    console.log(`RPO (Data Loss Window): ${healthStatus.rtoRpoMetrics.dataLossWindow} hours`);
    ...
    console.log('2. Check health endpoint: curl http://localhost:3000/api/infrastructure/health');
    console.log('3. Review backup files in:', process.env.BACKUP_PATH || 'var/backups/mongodb');
