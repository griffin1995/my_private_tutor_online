...
export async function analyzeTrace() {
	...
	try {
		...
		console.log(
			'⚠️  No trace file found. Run "npm run dev:trace" first to generate trace data.',
		);
...
export async function analyzeBundle() {
	...
	try {
		...
		console.log(
			'✅ Bundle analysis complete! Check your browser for the interactive report.',
		);
	} catch (error) {
...
export async function profileBuild() {
	...
	try {
		...
		console.log(
			'   - .next/trace-turbopack (analyze with: npm run dev:analyze-trace)',
		);
	} catch (error) {
	...
	try {
		...
		try {
			...
			if (parseFloat(size) > 1000) {
				// If over 1GB
				console.log(
					'⚠️  Large node_modules detected. Consider cleaning unused dependencies.',
				);
			}
		} catch (error) {
		...
		console.error(
			'❌ Health check configuration analysis failed:',
			error.message,
		);
	}
