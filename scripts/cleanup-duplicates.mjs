...
async function cleanupFontDuplicates() {
	...
	try {
		...
		for (const entry of entries) {
			...
			const weight = entry.match(
				/(\d{3}|light|regular|medium|semibold|bold|extrabold)/i,
			);
...
async function optimizeLogoVariants(logoVariants) {
	...
	for (const logo of logoVariants) {
		...
		// Track sizes
		const sizeCategory =
			logo.size > 500000 ? 'large'
			: logo.size > 100000 ? 'medium'
			: 'small';
...
async function generateCleanupReport(
	...
	console.log(
		`Potential Savings: ${Math.round(analysisReport.potentialSavings / 1024)}KB`,
	);
	...
	if (duplicates.length > 0) {
		...
		duplicates.forEach((group, index) => {
			...
			const savingsPotential = group
				.slice(1)
				.reduce((sum, file) => sum + file.size, 0);
			console.log(
				`  💾 Potential savings: ${Math.round(savingsPotential / 1024)}KB`,
			);
		});
	} else {
	...
	logoAnalysis.formats.forEach((files, format) => {
		...
		console.log(
			`${format}: ${files.length} files, ${Math.round(totalSize / 1024)}KB`,
		);
	});
	...
	console.log(
		'• Consolidate logo variants to essential formats only (.svg, .png, .webp)',
	);
	console.log('• Remove unused font format duplicates (prefer .woff2)');
	console.log(
		'• Consider using a single logo format with different sizes rather than variants',
	);
	console.log(
		'• Implement proper asset versioning to prevent future duplicates',
	);
