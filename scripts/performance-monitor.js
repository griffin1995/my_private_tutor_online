...
const { PerformanceRegressionDetector } = require('../src/lib/monitoring/performance-regression-detector');
const { performanceAnalytics } = require('../src/lib/monitoring/performance-analytics');
...
class PerformanceMonitorCLI {
  ...
  async startMonitoring() {
    console.log(`${COLORS.cyan}🔍 Starting Performance Regression Monitoring${COLORS.reset}`);
    console.log(`${COLORS.white}Protecting £191,500/year performance value${COLORS.reset}\n`);
  ...
  async runBenchmark() {
    console.log(`${COLORS.cyan}📊 Running Performance Benchmark${COLORS.reset}\n`);
  ...
  async generateReport() {
    ...
    try {
      ...
      const reportPath = path.join(process.cwd(), 'logs/performance-reports/latest.json');
      ...
      this.stopSpinner(spinner);
      console.error(`${COLORS.red}❌ Report generation failed:${COLORS.reset}`, error.message);
      process.exit(1);
    }
  }
  ...
  async checkStatus() {
    ...
    try {
      ...
      // Overall status
      const statusIcon = status.status === 'excellent' ? '🏆' :
                        status.status === 'good' ? '✅' :
                        status.status === 'needs-improvement' ? '⚠️' : '❌';

      console.log(`${statusIcon} Overall Status: ${COLORS.bold}${status.status.toUpperCase()}${COLORS.reset}`);
      console.log(`Performance Score: ${this.getScoreColor(status.score)}${status.score}/100${COLORS.reset}`);
      console.log(`Last Update: ${status.lastUpdate.toLocaleString()}`);

      if (status.criticalAlerts > 0) {
        console.log(`${COLORS.red}⚠️ Critical Alerts: ${status.criticalAlerts}${COLORS.reset}`);
      }

      // Performance metrics
      if (perfStatus.latestBenchmark) {
        console.log('\n📊 Latest Metrics:');
        console.log(`  • Build Time: ${this.formatDuration(perfStatus.latestBenchmark.buildTime)}`);
        console.log(`  • Bundle Size: ${this.formatSize(perfStatus.latestBenchmark.bundleSize)}`);
        console.log(`  • Memory Usage: ${this.formatSize(perfStatus.latestBenchmark.memoryUsage)}`);
      }

      // Recent trend
      if (perfStatus.regressionSummary.recentTrend !== 'stable') {
        const trendIcon = perfStatus.regressionSummary.recentTrend === 'improving' ? '📈' : '📉';
        console.log(`\n${trendIcon} Recent Trend: ${perfStatus.regressionSummary.recentTrend}`);
      }

      // Business value protection
      console.log(`\n💰 Protected Value: £191,500/year`);
      console.log(`🛡️ Performance Shield: ${status.score >= 95 ? 'ACTIVE' : 'AT RISK'}`);
  ...
  async showDashboard() {
    ...
    console.log(`${COLORS.bold}🚀 PERFORMANCE MONITORING DASHBOARD${COLORS.reset}`);
    console.log(`${COLORS.cyan}${'='.repeat(60)}${COLORS.reset}\n`);
    ...
    if (perfStatus.latestBenchmark) {
      ...
      // Build Time
      const buildTimeStatus = b.buildTime <= PERFORMANCE_TARGETS.buildTime ? '✅' :
                            b.buildTime <= PERFORMANCE_TARGETS.buildTime * 1.1 ? '⚠️' : '❌';
      console.log(`${buildTimeStatus} Build Time: ${this.formatDuration(b.buildTime)} (Target: ${this.formatDuration(PERFORMANCE_TARGETS.buildTime)})`);

      // Bundle Size
      const bundleStatus = b.bundleSize <= PERFORMANCE_TARGETS.bundleSize ? '✅' :
                          b.bundleSize <= PERFORMANCE_TARGETS.bundleSize * 1.07 ? '⚠️' : '❌';
      console.log(`${bundleStatus} Bundle Size: ${this.formatSize(b.bundleSize)} (Target: ${this.formatSize(PERFORMANCE_TARGETS.bundleSize)})`);

      // Memory Usage
      const memStatus = b.memoryUsage <= 1073741824 ? '✅' :
    ...
    console.log(`Protection Status: ${status.score >= 95 ? '🛡️ SECURED' : '⚠️ AT RISK'}`);
    ...
    if (perfStatus.buildHistory.length > 0) {
      ...
      for (const build of recentBuilds) {
        ...
        console.log(`${icon} ${time} - ${this.formatDuration(build.duration)} - ${build.stages.length} stages`);
      }
    }
    ...
    if (status.score >= 95) {
      console.log('🏆 Excellent performance! Continue monitoring for regressions.');
    } else {
      console.log('⚡ Run full performance report for optimization recommendations.');
  ...
  async protectPerformance() {
    ...
    if (benchmark.buildTime > PERFORMANCE_TARGETS.buildTime * 1.1) {
      issues.push(`Build time regression: ${this.formatDuration(benchmark.buildTime)}`);
      protectionStatus = 'AT RISK';
    }

    if (benchmark.bundleSize > PERFORMANCE_TARGETS.bundleSize * 1.07) {
      issues.push(`Bundle size increase: ${this.formatSize(benchmark.bundleSize)}`);
      protectionStatus = 'AT RISK';
    }
    ...
    const statusColor = protectionStatus === 'ACTIVE' ? COLORS.green :
                       protectionStatus === 'AT RISK' ? COLORS.yellow : COLORS.red;

    console.log(`${statusColor}${'='.repeat(60)}${COLORS.reset}`);
    console.log(`${COLORS.bold}PROTECTION STATUS: ${statusColor}${protectionStatus}${COLORS.reset}`);
    console.log(`${statusColor}${'='.repeat(60)}${COLORS.reset}\n`);
  ...
  displayReportSummary(report) {
    ...
    console.log('\nBuild Metrics:');
    console.log(`  • Average Build Time: ${this.formatDuration(report.buildMetrics.averageBuildTime)}`);
    console.log(`  • Success Rate: ${report.buildMetrics.successRate.toFixed(1)}%`);

    console.log('\nBundle Metrics:');
    console.log(`  • Current Size: ${this.formatSize(report.bundleMetrics.currentSize)}`);
    console.log(`  • Size Change: ${report.bundleMetrics.sizeChange > 0 ? '+' : ''}${this.formatSize(report.bundleMetrics.sizeChange)}`);
    ...
    console.log('\nBusiness Impact:');
    console.log(`  • User Experience Score: ${report.businessImpact.userExperienceScore}/100`);
    console.log(`  • Competitive Advantage: ${report.businessImpact.competitiveAdvantage}`);
  ...
  displayPerformanceGauge(score) {
    ...
    const color = score >= 90 ? COLORS.green :
                 ...
                 score >= 50 ? COLORS.magenta : COLORS.red;

    console.log(`Performance Score: ${color}${score}/100${COLORS.reset}`);
    console.log(`[${color}${'█'.repeat(filledWidth)}${COLORS.reset}${'░'.repeat(emptyWidth)}]`);
  }
  ...
  checkTargets(benchmark) {
    ...
    checks.forEach(check => {
      ...
      console.log(`${icon} ${check.name}: ${color}${check.formatter(check.current)}${COLORS.reset} (target: ${check.formatter(check.target)})`);
