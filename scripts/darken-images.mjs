...
async function darkenImage(inputPath, outputPath, brightnessFactor) {
  try {
    ...
    console.log(`✅ Darkened: ${path.basename(inputPath)} → ${path.basename(outputPath)}`);
    return true;
  } catch (error) {
...
async function main() {
  try {
    ...
    console.log(`🔅 Brightness: ${brightness} (${Math.round((1 - brightness) * 100)}% darker)`);
    ...
    if (successCount > 0) {
      ...
      console.log('💡 You can now use the darkened images instead of CSS overlays.');
