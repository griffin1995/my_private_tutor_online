	...
	const analyzeDirectory = async (dir, relativePath = '') => {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				// Track by format
				const format =
					fileInfo.format in analysis.byFormat ? fileInfo.format : 'other';
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				try {
					...
					console.log(
						`🔄 Optimizing: ${path.relative(IMAGES_DIR, inputPath)} (${Math.round(originalSize / 1024)}KB)`,
					);
...
async function generateReport(analysis, optimizationResults) {
	...
	Object.entries(analysis.byFormat).forEach(([format, data]) => {
		if (data.count > 0) {
			console.log(
				`${format.toUpperCase()}: ${data.count} files, ${Math.round(data.size / (1024 * 1024))}MB`,
			);
		}
	});
	...
	if (optimizationResults.processed > 0) {
		const totalSavings =
			optimizationResults.originalSize - optimizationResults.optimizedSize;
		const savingsPercent = Math.round(
			(totalSavings / optimizationResults.originalSize) * 100,
		);

		console.log('\n✅ OPTIMIZATION RESULTS:');
		console.log(`Files Processed: ${optimizationResults.processed}`);
		console.log(
			`Original Size: ${Math.round(optimizationResults.originalSize / (1024 * 1024))}MB`,
		);
		console.log(
			`Optimized Size: ${Math.round(optimizationResults.optimizedSize / (1024 * 1024))}MB`,
		);
		console.log(
			`Total Savings: ${Math.round(totalSavings / (1024 * 1024))}MB (${savingsPercent}%)`,
		);
	...
	if (analysis.largeFiles.length > 0) {
		console.log(
			'• Prioritize optimizing files >5MB for immediate performance gains',
		);
	}
	...
	console.log(
		'• Implement responsive images with Next.js Image component sizes prop',
	);
...
async function main() {
	try {
		...
		console.log(
			'Enterprise-grade image optimization for royal client standards\n',
		);
		...
		console.log(
			'💡 Next steps: Deploy optimized assets and monitor performance improvements',
		);
	} catch (error) {
