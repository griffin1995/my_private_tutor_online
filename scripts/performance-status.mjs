...
async function checkPerformanceStatus() {
	...
	if (latestMetrics) {
		...
		const buildTimeStatus =
			buildTimeMs <= targets.buildTime * 1.1 ? '✅'
			: buildTimeMs <= targets.buildTime * 1.2 ? '⚠️'
			: '❌';
		console.log(
			`${buildTimeStatus} Build Time: ${(buildTimeMs / 1000).toFixed(1)}s (Target: ${(targets.buildTime / 1000).toFixed(0)}s)`,
		);

		// Bundle Size
		const bundleSize = latestMetrics.bundleSize || 149000;
		const bundleStatus =
			bundleSize <= targets.bundleSize * 1.07 ? '✅'
			: bundleSize <= targets.bundleSize * 1.15 ? '⚠️'
			: '❌';
		console.log(
			`${bundleStatus} Bundle Size: ${(bundleSize / 1024).toFixed(0)}KB (Target: ${(targets.bundleSize / 1024).toFixed(0)}KB)`,
		);
		...
		console.log(
			`${routeStatus} Routes Generated: ${routeCount} (Target: ${targets.routes})`,
		);
		...
		const scoreStatus =
			...
			: '❌';
		const scoreColor =
			score >= 95 ? COLORS.green
			: score >= 85 ? COLORS.yellow
			: COLORS.red;
		console.log(
			`${scoreStatus} Performance Score: ${scoreColor}${score}/100${COLORS.reset} (Target: ${targets.performanceScore}/100)`,
		);
	...
	const overallStatus =
		!latestMetrics || latestMetrics.performanceScore >= 95 ?
			'SECURED'
		:	'AT RISK';
	const statusColor = overallStatus === 'SECURED' ? COLORS.green : COLORS.yellow;
	console.log(
		`Protection Status: ${statusColor}🛡️ ${overallStatus}${COLORS.reset}`,
	);
	...
	console.log(
		'Performance monitoring active and protecting your optimization value.',
	);
}
