...
export function RoyalTrustIndicators({ 
  ...
  // CMS DATA SOURCE: Using getTrustIndicators for premium trust indicators
  ...
  const containerClasses = {
    horizontal: 'bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300',
    vertical: 'bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300',
    grid: 'bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300',
    premium: 'bg-gradient-to-br from-white to-primary-50 rounded-2xl p-8 shadow-premium hover:shadow-royal transition-all duration-500 border border-primary-100 group',
  }

  const itemClasses = {
    horizontal: 'flex flex-col sm:flex-row items-center gap-3 text-center sm:text-left',
  ...
  const iconContainerClasses = {
    ...
    premium: 'flex-shrink-0 p-4 rounded-full bg-gradient-to-br from-accent-400 to-accent-500 group-hover:from-royal-400 group-hover:to-royal-500 transition-all duration-500 shadow-lg',
  }
  ...
  const iconClasses = {
    ...
    premium: 'h-8 w-8 text-white group-hover:scale-110 transition-transform duration-300',
  }
      ...
      {indicators.map((indicator, index) => {
        ...
        return (
          <div 
            ...
            aria-describedby={showDescription ? `trust-indicator-${index}-desc` : undefined}
          >
...
// Export variant props for storybook/documentation
export type RoyalTrustIndicatorsVariant = 'horizontal' | 'vertical' | 'grid' | 'premium'